...
> **写数字的纪律**（2026-09-27 加）：文档 / 片子里的**每一个数字都必须现测**并在旁边给出可复现的命令或脚本；
> 引用别处的数字前重测、写明口径（字符数还是字节数、语句数还是调用点、实测还是估算）。
> 实测过的教训：「契约 33 条」（真值 92→119）、「应用 SQL 约 297 条」（真值 336 处调用点）。
> 正本见 `design-plus/验证门禁.md` §七之三。
...
- 数据库默认 SQLite；配 `DATABASE_URL` 即切 PostgreSQL，建表/SQL 方言自动翻译，应用零改动。
  ...
  **唯一路径**（2026-09-26 切为默认、2026-09-27 删掉 `legacy` 退回开关；回退手段改为 git，见 `CONVENTION.md` §14.3）；
...
- 知识库检索的**重排（rerank）显式关掉**（2026-09-27）：lightrag 默认 `enable_rerank=True` 而平台没配
  `rerank_model_func` ⇒ 每次查询打一行"no rerank model is configured"的 warning，**而它什么都不做**
  （日志证据 `84 -> 84 (deduplicated 0)`）。已在 `knowledge_graph.query()` 里显式传 `enable_rerank=False`。
  **要开需两处一起改**：`QueryParam(enable_rerank=True)` + `_get_rag` 传 `rerank_model_func=`；
  代价：API 路线（lightrag 自带 ali/jina/cohere/generic，需一家 key，DeepSeek 无此接口）或
  本地 `bge-reranker-base`（≈1.1 GB 下载 + 1.1 GB 内存，2核2G 服务器跑不了）。
