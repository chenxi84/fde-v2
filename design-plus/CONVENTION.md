...
> 本节记录"为什么这么做"。应用开发者不需要读它 ——
> 应用侧看到的一切（`self.db.execute(裸 SQL, ?)`、`schema.sql` 是唯一真相源）都没有变。
>
> **为什么现在做（而不是按原计划等 3.0）**：FDE v2 原本定位**小型环境**（演示 / 部门级），
> DB 层增强**原计划排在 3.0 之后**；但已有反馈希望把本项目用于**数据可靠性要求较高的生产环境**，
> 于是把这块**提前**到 2.4 落地。这也是本版把"验证面"做得很重的直接原因 ——
> 生产环境里"本地全绿"不算数（§14.5 的克隆库配方、`scripts/verify_pg_live.py` 的现场探针都是为此加的）。
