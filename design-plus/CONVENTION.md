...
- **可选设计文档 `应用详设.md` / `前端详设.md`**（及组级 `app/<组>/架构设计.md`、`app/<组>/前端详设/`）：构建流水线（九步法，见 `design-plus/工具链使用说明.md`）
