...
- **依赖输入**: 版本（**主数据**：`md_monthly_version.get` 产生）；物料清单（**主数据**：`md_material.list(status=正常)` 产生）；客户（**上游**：`sales_history.purchasing_customers` 产生）
- **输出**: `sales_forecast_line` / `sales_forecast_summary` 行（三者同 `version_no`）
- **生效点**: `sales_forecast.open_version`（清单 + 处理表行同事务写入）；`summarize` 刷新汇总表

#### 3.2.2 `BR-02` 处理表行唯一
- **规则描述**: 处理表行按（版本 + 物料 + 客户 + 滚动月度）唯一。
- **适用场景**: open_version、fill_customer 等所有对处理表行的写操作。
- **示例**: 同一版本同一物料同一客户同一滚动月度只能有一行，不可重复生成。
- **依赖输入**: `version_no` + `material_no` + `customer_no` + `rolling_month`（入参 + 清单行）
- **输出**: `sales_forecast_line` 主键（`version_no` + `material_no` + `customer_no` + `rolling_month`）唯一
- **生效点**: `sales_forecast.open_version`、`sales_forecast.create`（先查后插，命中即拒绝）
...
- **依赖输入**: 处理表 `final_qty`（**本应用**：`decide` / `set_final` 产生）
- **输出**: `sales_forecast_summary.final_qty_sum`
- **生效点**: `sales_forecast.summarize`（DELETE + 逐行 INSERT 整体重算，无独立编辑入口）
...
- **依赖输入**: 正常状态物料（**主数据**：`md_material.list(status=正常)` 产生）；该物料的历史采购客户（**上游**：`sales_history.purchasing_customers(material_no)` 产生）
- **输出**: `sales_forecast_line` 的「物料×客户」组合行（`material_no` + `customer_no`；无历史客户时 `customer_no` 为空、仅一行）
- **生效点**: `sales_forecast.open_version` → `md_material.list` + `sales_history.purchasing_customers`

#### 3.2.5 `BR-05` 每个组合拆 N+1/N+2/N+3 三行进处理表
- **规则描述**: 清单每个「物料×客户」组合在指定月度版本下，按滚动月度拆成 N+1、N+2、N+3 三行进入处理表（空客户组合同样拆三行）。
- **适用场景**: open_version。
- **示例**: 4 行清单 → 12 行处理表。
- **依赖输入**: `ROLLING_MONTHS`（**本应用**常量：N+1/N+2/N+3）
- **输出**: `sales_forecast_line.rolling_month`（每个组合 3 行）
- **生效点**: `sales_forecast.open_version`、`sales_forecast.create`
...
- **依赖输入**: `md_monthly_version.lock_status`（**主数据**：`md_monthly_version` 产生）
- **输出**: 无（拒绝：草稿期的写操作之外一律抛 `FdeError`）
- **生效点**: `sales_forecast._require_draft` ← 被 `open_version` / `fill_customer` / `calc_baseline` / `adjust_event` / `decide` / `set_final` / `summarize` 共同调用

#### 3.2.7 `BR-07` 填写客户预测前校验物料存在
- **规则描述**: fill_customer 的 material_no 须通过 `md_material.get` 校验存在，否则拒绝。
- **适用场景**: fill_customer。
- **示例**: material_no=NOTEXIST → 拒绝"物料不存在"。
- **依赖输入**: 物料是否存在（**主数据**：`md_material.get` 产生）
- **输出**: 无（拒绝）
- **生效点**: `sales_forecast.fill_customer` → `md_material.get`；`create` → `md_material.get`

#### 3.2.8 `BR-08` 填写客户预测前校验客户存在
- **规则描述**: fill_customer 的 customer_no 须通过 `md_customer.get` 校验存在，否则拒绝。
- **适用场景**: fill_customer。
- **示例**: customer_no=NOTEXIST → 拒绝"客户不存在"。
- **依赖输入**: 客户是否存在（**主数据**：`md_customer.get` 产生）
- **输出**: 无（拒绝）
- **生效点**: `sales_forecast.fill_customer` → `md_customer.get`（`create` 同样校验，客户非空时）

#### 3.2.9 `BR-09` 原始需求数量可空
- **规则描述**: orig_qty 为客户填报值，客户不填报时为空；为空时调整后需求亦为空，后续决策走"MAPE 不存在/默认信基线"路径。
- **适用场景**: fill_customer。
- **示例**: 客户未提供预测 → orig_qty 空。
- **依赖输入**: `orig_qty`（入参，**可空**——客户不填报时为空）
- **输出**: `sales_forecast_line.orig_qty`（可空）、`adj_qty`（orig 为空时同为空）
- **生效点**: `sales_forecast.fill_customer`
...
- **依赖输入**: 历史预测合计与历史实际合计（**上游**：`attainment` 产生，经 `attainment.get` 取 `bias`）
- **输出**: `sales_forecast_line.bias`
- **生效点**: `sales_forecast.fill_customer` → `attainment.get`（取不到时 bias 留空，`adj = orig`）

#### 3.2.11 `BR-11` 调整后需求计算
- **规则描述**: 调整后需求数量 = 原始需求数量 × (1 − bias)。系统自动修正，人工可调。
- **适用场景**: fill_customer。
- **示例**: orig_qty=1000、bias=+0.1 → adj_qty = 1000 × 0.9 = 900。
- **依赖输入**: `orig_qty`（入参）；`bias`（**上游**：`attainment.get` 产生，缺失按 0 计）
- **输出**: `sales_forecast_line.adj_qty`（= `orig_qty` × (1 − bias)，人工可覆写）
- **生效点**: `sales_forecast.fill_customer`
...
- **依赖输入**: 基线值 `base_qty`（**本应用**：`calc_baseline` 产生）；客户值 = 该品种全部客户 `adj_qty` 合计（**本表**，`fill_customer` 产生）
- **输出**: 无（中间量：决定 `abnormal_flag` 与 `final_qty` 的取值分支）
- **生效点**: `sales_forecast.decide` → `_deviation`

#### 3.2.13 `BR-13` MAPE 阈值判定
- **规则描述**: MAPE ≤ 阈值（可配置，默认 20%）视为「小」（客户可信）；MAPE > 阈值视为「大」（客户不可信）。
- **适用场景**: decide。
- **示例**: MAPE=0.10 ≤ 0.20 → 小；MAPE=0.30 > 0.20 → 大。
- **依赖输入**: `mape`（**上游**：`attainment.get` 产生）；阈值 `MAPE_THRESHOLD`（**本应用**）
- **输出**: 无（中间量：决定 BR-15 取「客户调整后值」还是「基线和事件合计量」）
- **生效点**: `sales_forecast.decide`（MAPE ≤ 20% 走客户值，否则走基线）

#### 3.2.14 `BR-14` 异常标记规则
- **规则描述**: MAPE 存在 且 偏离率 > 5% → 标记异常（abnormal_flag=true）；MAPE 不存在 或 偏离率 ≤ 5% → 不标记（abnormal_flag=false）。
  **兜底闸（2026-09-19 补）**：决策走完后若 `final_qty` 仍为空、又尚未标异常 ⇒ **一律标异常**。
- **适用场景**: decide。
- **计算方式**:
  - MAPE 小 且 偏离率 > 5% → 标记异常
  - MAPE 大 且 偏离率 > 5% → 标记异常
  - MAPE 不存在 → 不标记
  - 其余（偏离率 ≤ 5%）→ 不标记
  - **出口闸**：`final_qty is None and not abnormal_flag` → 标记异常（无论走的是哪条分支）
- **示例**: 偏离率 8% 且 MAPE 存在 → 标记异常；偏离率 2% → 不标记；
  **客户未报数且物料无历史（算不出基线）→ 标记异常**（两源皆空，系统无话可说，须转人工）。
- **⚠ 实现缺口与修复（2026-09-19 记录）**：本规则此前**没有出口闸** —— `decide` 的**每一条分支都可能落到
  `final_qty = None`**（客户侧不可用且基线也空、无基线可比且本行 `adj_qty` 空、MAPE 两档取到的值本身为空……），
  这些情况会被**安静地留空**，而 `summarize` 用 `COALESCE(final_qty, 0)` 合计 ⇒
  该物料在毛需求里**静默变成 0、从计划里消失**，且 `abnormal_flag=0`、界面上没有任何提示。
  实测（版本 202611 / `BYD-HAN-FB27`：ERP 同步进来的改款新料，无前序链、无断点、客户未报数）：
  N+1/N+2/N+3 三行全部 `abnormal_flag=0` 且 `final_qty` 为空。
  这与 **B-01 同族不同路径**：B-01 是「基线算好了却被丢弃」，本条是「两个来源都空，系统无话可说、也没说自己无话可说」。
  口径：**不能替人决定的时候，必须说出来** —— 与「两源分歧过大」在界面上同为「待人工」，只是原因可分辨。
  这正是 `宣传/演示业务方案.md` §2.5 对 FB27 写明的期望（「如实标记数据不足，预测留空待人工」——
  「留空」早就做到了，「标记」此前没做）。**发现方式**：`scripts/verify_psc_oracles.py` 的 C7 判据。
- **⚠ 前提缺口与修复（2026-09-20 记录）**：兜底闸加上了，却**够不到它要保护的那类行**。
  `open_version` 对「没有任何历史采购记录」的物料只初始化一行、**客户留空**
  （否则 N 个客户 × 3 个月会把表铺满），这类空客户行是**合法数据**；但 `decide` / `set_final` / `get` /
  `calc_baseline` / `adjust_event` 的第一句都是 `_require(customer_no, "客户编号")` ⇒ 空串直接抛错，
  而 `decide_batch` 对单行失败是「跳过」。于是这类行**瘫在表里**：算不出、标不了异常、连详情都打不开，
  最终仍在汇总里变成 0 —— 兜底闸写了，闸门却在它上游。
  实测（202611 / `BYD-HAN-FB27` 三行）：`decide_batch` 报 `failed`、三行 `abnormal_flag=0` 且 `final_qty` 为空。
  **修法**：行级动作的客户键改用 `_clean_customer_key`（**允许为空**）——行身份本就是
  「版本 + 物料 + 客户 + 滚动月度」四元组，客户可以为空，真正的校验在 `_get_line`（行不存在即抛）。
  `fill_customer` 仍要求非空（给一个没有客户的行「填报客户预测」没有意义）。
  `calc_baseline` 对这类行返回 `base_qty=None`（无历史本就该为空），语义不变。
  **发现方式**：C7 判据**第一次拿到非空分母**（此前长期是「分母为空」的假绿），一触发就报出这三行。
  ⚠ 同一批数据还暴露出 **A15 与 C7 口径打架**：A15 停在兜底闸加入之前的语义
  （「客户值与基线都取不到 ⇒ 不标记」），与 C7「非异常行必须有最终预测」对同一批空客户行给出**相反期望** ——
  两条判据谁也没错，是口径没对齐。已按 BR-14 现文把 A15 改为**并集**（BR-12 偏离率规则 ∨ 兜底闸）。
  回归：`TC-ERR-45`（守「够得到」；`TC-ERR-44` 用的是有客户的行，缺陷期间照样通过，两条缺一不可）。
- **依赖输入**: `mape`（**上游**：`attainment.get` 产生）；偏离率（由 `base_qty` + `adj_sum` 现算）
- **输出**: `sales_forecast_line.abnormal_flag`
- **生效点**: `sales_forecast.decide` / `decide_batch`（后者的出口闸经 `decide` 逐行生效）
...
- **规则描述**: 按 MAPE 大小与偏离率决定最终预测量：
  ...
  - 客户侧不可用（无人填报原始预测 → 偏差率无从计算）→ 取【基线和事件合计量】（同「默认信基线」）
...
- **⚠ 实现缺口与修复（2026-09-15 记录）**：本规则原有**两个分支未实现** —— `decide` 的三个分支一律取 `adj_qty`，从不取【基线和事件合计量】，`base_event_qty` 字段全文件**只写不读**；`MAPE_THRESHOLD`（第 19 行已定义、README 已写明）**无一行代码引用**。后果：客户未报数时 `final_qty` 留空，而 `summarize` 把空按 0 合计 ⇒ 算好的基线被静默丢弃、毛需求只剩库存水位层。实测（版本 202610）`M9-BEAM` 基线 1967.71/月、`BYD-HAN-SPARM` 45.30/月全部丢失，三个月合计 **6025.88 件**未进毛需求，且 `abnormal_flag=0`、界面无任何提示。已按本规则补齐四分支（含新增的「客户侧不可用」一档）；负基线钳到 0（预测为负无业务含义）。
- **发现方式**: `scripts/verify_psc_oracles.py` 的 A13 对账（基线非零却未进最终值）。
- **依赖输入**: `mape`（**上游**：`attainment.get`）；`adj_qty`（**本表**：`fill_customer` 产生）；`base_event_qty`（**本表**：`calc_baseline` + `adjust_event` 产生）
- **输出**: `sales_forecast_line.final_qty`（异常行留空；基线为负时钳 0）
- **生效点**: `sales_forecast.decide` / `decide_batch`

#### 3.2.16 `BR-16` 偏离/误差阈值与 MAPE 阈值可配置
- **规则描述**: 偏离阈值（默认 5%）与 MAPE 阈值（默认 20%）均为可配置参数，不写死，避免调优改代码。
- **适用场景**: decide。
- **示例**: 配置偏离阈值 5%、MAPE 阈值 20% 后，决策按配置生效。
- **依赖输入**: `MAPE_THRESHOLD` / `DEVIATION_THRESHOLD`（**本应用**）
- **输出**: 无（影响 BR-13 / BR-14 的判定结果）
- **生效点**: `sales_forecast.decide`（⚠ 现状是**类常量**，改阈值等于改代码 —— 「可配置」目前只到「集中在一处」这一层，没有运行期配置源）

#### 3.2.17 `BR-17` 基线数量计算
- **规则描述**: 基线数量 = 基线方法 + 基线参数（base_params，JSON）作用于历史干净需求的结果。
- **适用场景**: calc_baseline。
- **示例**: 移动平均 window=3、历史 [100,110,120] → base_qty=110。
- **依赖输入**: `base_method` / `base_params`（**主数据**：`md_material` 产生）；历史干净需求（**上游**：`sales_history.history_sequence` 产生）
- **输出**: `sales_forecast_line.base_qty`（未配置基线方法时留空，不报错）
- **生效点**: `sales_forecast.calc_baseline` → `md_material.get` + `sales_history.history_sequence`

> ### ⚠ BR-18 ~ BR-21（移动平均 / 指数平滑 / 阶跃检测 / 借用参考）已退役
>
> 这四个方法**已从自动选型退役**：`strategy_fitting._predict_fit` 只竞标 `AutoTheta / AutoARIMA /
> AutoETS`（常规需求）与 `CrostonOptimized / TSB`（间歇需求）五个，**不再产出这四个名字**
> （业务口径见 `销售预测与库存策略说明.md` §1.2）。
>
> **退役原因（不是被否决）**：`AutoETS` 本身就是指数平滑（ETS 即误差-趋势-季节的状态空间形式）、
> `AutoTheta` 本质是带漂移的 SES —— **指数平滑家族并没有缺席，它以自动选参的形式在跑**；
> 候选池里再放一个固定参数的手写 Holt-Winters，等于让同一家族的两个实现做不公平比较。
> 「借用参考」当年要解决的「新件无历史」，现已由**断点追溯 + 前序物料链**承担。
>
> **现状（2026-09-19 起）**：
> - **前端**：`md_material` 的「基线方法」下拉**只列自动拟合会产出的 6 个**；
>   编辑历史数据时，旧值以「（已退役）XXX」显示（`view.js::isRetiredMethod`），避免一保存把旧值抹掉。
> - **后端**：`md_material._BASE_METHODS` **仍接受这 4 个值**，`_calc_base_qty` 的四个分支也保留 ——
>   历史数据与**批量导入**不受影响。所以它们**不是删除，是退役**。
> - **顺带查实的两处实现缺口**（下面 BR-20 / BR-21 各自标注）：
>   **BR-20 的计算方式从未实现**（该分支实际等同移动平均），**BR-21 的 `mode` / `offset` 从未被使用**。
>   这两条都是「声明了、能填、能跑，但语义是空的」——留着下拉入口会让人以为在选一种能力，
>   实际拿到的是另一种算法的结果，且测试抓不出来。

#### 3.2.18 `BR-18` 移动平均（MA）参数约束

> **状态：已退役**（不再由自动拟合产出；仅手工/导入的历史数据可达）。实现本身是完整的。
...
- **示例**: `{"window": 6}`。
- **依赖输入**: `base_params.window`（**主数据**：`md_material.base_params` 产生，参数本身由 `md_material._normalize_base_params` 校验）
- **输出**: `sales_forecast_line.base_qty`（近 window 期均值）
- **生效点**: `sales_forecast.calc_baseline` → `_calc_base_qty`（移动平均分支）

#### 3.2.19 `BR-19` 指数平滑（ES）参数约束

> **状态：已退役**（不再由自动拟合产出；仅手工/导入的历史数据可达）。
> 实现本身**完整**：`_es_forecast` 三个分支都在（水平 SES / 趋势 Holt / 季节 Holt-Winters 加法），
> 而且是传统四法里**唯一按 horizon 真正向前投影**的一个（其余三个是水平法、各滚动月度同值）。
> 退役的是「这个入口」，不是「指数平滑这个家族」——它现在以 `AutoETS` 的形式在自动选型里跑。
...
- **依赖输入**: `base_params`（alpha/trend/beta/seasonal/gamma/period，**主数据**：`md_material` 产生）
- **输出**: `sales_forecast_line.base_qty`（按 trend/seasonal 决定水平平推 / Holt / Holt-Winters）
- **生效点**: `sales_forecast.calc_baseline` → `_es_forecast`

#### 3.2.20 `BR-20` 阶跃检测（Step Detection）参数约束

> **状态：已退役，且下面的「计算方式」从未实现。**
> `sales_forecast._calc_base_qty` 的阶跃检测分支实际只有一句 `history[-lookback:]` 求均值 ——
> **与移动平均逐字等价**，只是参数名从 `window` 换成了 `lookback`。
> `threshold`（变化率阈值）与 `confirm_periods`（持续期数）**从头到尾没有被读取过一次**
> （`md_material` 侧仍在校验它们的取值范围，所以「填得进去、存得下来、算的时候不算它」）。
> **实战后果**：计划员在下拉里选「阶跃检测」，以为自己选了突变点识别，拿到的是移动平均的结果，
> 界面上没有任何提示，用例也没断言过它的输出 ⇒ 测试抓不出来。**这正是它必须撤出下拉的原因。**
> 突变/换代的场景现在由**断点追溯 + 前序物料链**承担（见 BR-11 / `md_breakpoint`）。
...
- **适用场景**: calc_baseline（base_method=阶跃检测）。**已退役**。
- **计算方式**: **（本行为文档原设计，代码未实现）** 回溯 lookback 期估计当前水平 → 检测相邻期相对变化率 (D_t − D_{t-1}) ÷ D_{t-1} 是否超 threshold → 持续 confirm_periods 期才认定永久阶跃 → 基线切换到阶跃后水平。
  **代码实际**：`round(sum(history[-lookback:]) / len(history[-lookback:]), 4)`，即回溯期算术平均，与 BR-18 移动平均同式。
- **示例**: `{"threshold":0.3,"confirm_periods":2,"lookback":6}`。
- **依赖输入**: `base_params`（threshold/confirm_periods/lookback，**主数据**：`md_material` 产生）
- **输出**: `sales_forecast_line.base_qty`（**实际为**回溯 lookback 期水平）
- **生效点**: `sales_forecast.calc_baseline` → `_calc_base_qty`（阶跃检测分支）

#### 3.2.21 `BR-21` 借用参考（Reference）参数约束

> **状态：已退役；且 `mode` / `offset` 两个参数从未被使用。**
> `_calc_base_qty` 的借用参考分支只做了三件事：取 `ref_material` → 取其历史均值 → `× scale`。
> **不论 `mode` 填 trend / season / lifecycle，都返回同一个水平均值；`offset` 被完全忽略。**
> （前端参数模板也照此收窄，不再列这两个参数 —— 写了不生效的参数比没有更误导。）
> 它当年要解决的「新件没有历史」，现在由**断点追溯 + 前序物料链**承担（BR-11 / `md_breakpoint`），
> 所以退役；但**后端仍接受这个值**，历史数据与批量导入不受影响。
...
- **依赖输入**: `base_params.ref_material` / `scale`（**主数据**：`md_material` 产生）；参考物料的**历史序列**（**上游**：`sales_history.history_sequence` 产生）
- **输出**: `sales_forecast_line.base_qty`（= 参考物料历史均值 × scale）
- **生效点**: `sales_forecast.calc_baseline` → `_calc_base_qty` → `_load_sales_history`
...
- **依赖输入**: `event_adj`（入参）
- **输出**: `sales_forecast_line.event_adj`、该行 `base_event_qty`（**只改归属期那一行**）
- **生效点**: `sales_forecast.adjust_event`
...
- **依赖输入**: `base_qty`（**本应用**：`calc_baseline` 产生）；`event_adj`（入参）
- **输出**: `sales_forecast_line.base_event_qty`
- **生效点**: `sales_forecast.calc_baseline`、`sales_forecast.adjust_event`（两处都重算）
...
- **依赖输入**: 断点链（**上游**：`md_breakpoint.trace` 产生）；切换时间（**上游**：`md_breakpoint.get_switch_time` 产生）；拼接后的历史（**上游**：`sales_history.history_sequence` 产生）
- **输出**: `sales_forecast_line.base_qty`（用拼接序列算）、`bp_material_no`、`switch_time`
- **生效点**: `sales_forecast.calc_baseline` → `md_breakpoint.trace` + `get_switch_time`（断点不存在时回落原物料，不报错）

#### 3.2.25 `BR-25` 汇总表合计规则
- **规则描述**: 汇总表 = 同物料 × 月度版本 × 滚动月度下所有客户的最终预测量（final_qty）合计。
- **适用场景**: summarize。
- **示例**: M1/N+1 下 C1=900、C2=300 → final_qty_sum=1200。
- **依赖输入**: 处理表 `final_qty`（**本表**：`decide` / `set_final` 产生；空按 0 计入）
- **输出**: `sales_forecast_summary.final_qty_sum`
- **生效点**: `sales_forecast.summarize`
...
- **依赖输入**: `abnormal_flag`（**本表**：`decide` 产生）
- **输出**: `sales_forecast_line.final_qty`（异常行**留空**，等人工填）
- **生效点**: `sales_forecast.decide`（异常分支不写值）；`set_final`（**仅异常行**允许人工填）

#### 3.2.27 `BR-27` 人工定稿优先（decide 不覆盖已定稿行）
- **规则描述**: `set_final` 定过的行会在**人工定稿台账** `sales_forecast_settle` 留一条记录；此后 `decide` / `decide_batch` 遇到该行**原样返回、不再重算**，`decide_batch` 在返回里报出 `settled_skipped`（计数）与 `settled_rows`（明细）。
- **为什么**: 人工定稿是本应用**唯一的人工决策窗口**（BR-26）。不设这条则定稿不可保——计划员刚在界面上定完稿，任何人（或产销协同链、或智能体）再点一次「批量决策」，`final_qty` 就被重算成 `None`、汇总归零，毛需求与主计划跟着归零，且**全程没有任何提示**。
- **定稿可改**: 重复调用 `set_final` 即覆盖（先删后插台账），不需要单独的撤销动作。
- **为什么单独一张表而不是加列**: 平台对应用库只做 `CREATE TABLE IF NOT EXISTS`，没有给已有表加列的机制；`sales_forecast_settle` 用 `IF NOT EXISTS` 对已有库也能自动建，零迁移风险；顺带留下 `settled_by`/`settled_at`（谁在什么时候定的稿）。
- **适用场景**: decide / decide_batch / set_final。
- **示例**: 异常行 set_final(950) → 再 decide_batch → `settled_skipped=1`，该行 `final_qty` 仍是 950。
- **依赖输入**: 人工定稿台账行（**本应用**：`sales_forecast_settle`，由 `set_final` 产生）
- **输出**: `sales_forecast_line.final_qty`（定稿后不被自动重算覆盖）；`decide_batch` 返回的 `settled_skipped` / `settled_rows`
- **生效点**: `sales_forecast.decide` → `_is_settled`（命中即原样返回）；`decide_batch`（计数并报出）
...
#### 3.3.8 数据8: 人工定稿台账 (sales_forecast_settle)
- **数据描述**: 人工定稿留痕。主键同处理行（版本+物料+客户+滚动月度），另存 `final_qty`（定稿值）、`settled_by`（定稿人 userno）、`settled_at`（定稿时间）。`decide` 靠它认人工作过的行（BR-27）。
- **数据类型**: 表
- **数据来源**: 人工动作 `set_final`（重复定稿即覆盖）
- **示例值**: 202608 / M1 / C001 / N+1 / 950 / admin / 2026-09-15 14:30:00
