...
- **依赖输入**: `final_qty_sum`（**上游**：`sales_forecast.summarize` 产生）；水位三层 A/C/B（**上游**：`inventory_strategy.get_water_level` 产生）
- **输出**: `demand.gross_qty`（= `forecast_qty` + `inventory_qty`）
- **生效点**: `demand.build_gross` → `sales_forecast.get_summary` + `inventory_strategy.get_water_level`

#### 3.2.2 `BR-02` 库存策略叠加口径
- **规则描述**: 每个滚动月度毛需求 = 销售预测 + 库存策略水位（A+C+B），N+1/N+2/N+3 同口径叠加（口径调整见 3.1 节说明）
- **水位取不到即报错（fail-closed）**: `build_gross` 逐物料取 `inventory_strategy.get_water_level`，**取不到就抛错中止，不按 0 处理**。旧行为是「无策略记录按水位 0 处理，不阻断整版本合成」——代价是**毛需求整层漏掉水位而调用方毫无察觉**：实测跑完产销协同链后 7 个物料的 `inventory_qty` 全是 0（应为 95.26 / 5209.32 …），毛需求只剩预测，主计划一路 0
- **流程顺序是硬要求**: 合成毛需求的节点**必须在库存策略节点之后**（`inventory_strategy.calc/calc_batch` 先跑）。产销协同链里 plan 节点因而声明 `depends_on: [forecast, inventory]`——此前只依赖 forecast，与 inventory 并行抢跑，正是上述现象的成因
- **适用场景**: build_gross 叠加库存策略
- **示例**: 目标水位 30 时 N+3 行 `inventory_qty = 30`；该物料若没有 202608 的水位策略，`build_gross` 直接报「未找到版本 202608 的库存水位策略（物料 X），无法合成毛需求」
- **依赖输入**: 水位三层 A/C/B（**上游**：`inventory_strategy.get_water_level` 产生；其**存在性**由 `inventory_strategy.calc_batch` 产生）—— 取不到即报错，不按 0 处理
- **输出**: `demand.inventory_qty`（= A+C+B）、`demand.gross_qty`
- **生效点**: `demand.build_gross` → `inventory_strategy.get_water_level`（流程顺序硬要求：`inventory_strategy.calc_batch` 节点必须先跑）

#### 3.2.3 `BR-03` 销售预测拆分口径
- **规则描述**: 销售预测按 N+1/N+2/N+3 **逐期拆分**进入各滚动月度行
- **适用场景**: build_gross 合成毛需求
- **示例**: 某物料汇总为 N+1=100 / N+2=80 / N+3=60，则三行 forecast_qty 分别为 100/80/60
- **依赖输入**: 汇总行的 `rolling_month` 维度（**上游**：`sales_forecast.get_summary` 产生）
- **输出**: `demand.forecast_qty`
- **生效点**: `demand.build_gross` → `sales_forecast.get_summary`
...
- **依赖输入**: `gross_qty`（**本表**，`demand.build_gross` 产生）；未发订单 / 当前库存 / 在途工单（**外部适配器**：`_load_open_order` / `_load_inventory` / `_load_in_transit` —— ⚠ 三者当前是**恒返回 `{}` 的 stub，系统内无产生方**）
- **输出**: `demand.net_qty`
- **生效点**: `demand.calc_net` → `_load_open_order` + `_load_inventory` + `_load_in_transit`
...
- **依赖输入**: `gross_qty`（**本表**）；版本状态 `lock_status`（**上游**：`md_monthly_version` 产生）
- **输出**: `demand.net_qty`（**只由 `calc_net` 写，无独立编辑入口**）
- **生效点**: `demand.publish` → `demand.calc_net`（同版本、同事务，禁跨版本拼接）

#### 3.2.6 `BR-06` 当前库存扣减口径
- **规则描述**: 当前库存 = 自有仓 + 寄售仓；寄售库存仍是我方资产，**必须扣减**
- **适用场景**: calc_net 扣减库存
- **示例**: 自有仓 30 + 寄售仓 10 = 当前库存 40，均须从毛需求中扣减
- **依赖输入**: 自有仓 + 寄售仓库存（**外部适配器**：`_load_inventory` —— ⚠ 当前是恒返回 `{}` 的 stub，系统内无产生方，故扣减项实测恒为 0）
- **输出**: `demand.onhand_qty`、`demand.net_qty`
- **生效点**: `demand.calc_net` → `_load_inventory`（**仅 N+1 行扣减**；N+2/N+3 记 0，见 BR-08）

#### 3.2.7 `BR-07` 在途工单扣减（防重复下单）
- **规则描述**: 在途工单（在制/在途）须从毛需求中扣减；漏扣在途会导致重复下单
- **适用场景**: calc_net 扣减在途
- **示例**: 已下达在制工单 15 件，须扣减，否则会再排产 15 件造成重复
- **依赖输入**: 在制 / 在途工单（**外部适配器**：`_load_in_transit` —— ⚠ 当前是恒返回 `{}` 的 stub，系统内无产生方）
- **输出**: `demand.in_transit_qty`、`demand.net_qty`
- **生效点**: `demand.calc_net` → `_load_in_transit`（**仅 N+1 行扣减**）

#### 3.2.8 `BR-08` 净需求各层非负
- **规则描述**: 净需求运算支持分层拆解（毛需求/未发/库存/在途逐层展示），且各层结果非负；`net_qty` 公式结果为负时置 0（已被现有库存/在途覆盖，无需生产）
- **适用场景**: calc_net 运算
- **示例**: gross=90、未发=10、库存=120、在途=0，净需求 = −20 → 置 0
- **依赖输入**: `gross_qty`（**本表**）；未发 / 库存 / 在途（**外部适配器**，见 BR-04）
- **输出**: `demand.net_qty`（负值钳 0）、`open_order_qty` / `onhand_qty` / `in_transit_qty`（N+2/N+3 记 0）
- **生效点**: `demand.calc_net`
...
- **依赖输入**: 生效替换关系（**上游**：`md_part_replace.list(status=生效)` 产生）；断点关系与切换时间（**上游**：`md_breakpoint.list` 产生）；物料×客户维度的预测（**上游**：`sales_forecast.get_summary` 产生）
- **输出**: `demand.material_no`（合并后的归属物料）、`demand.forecast_qty` / `gross_qty`
- **生效点**: 通用件合并：`sales_forecast.summarize`（按物料 SUM 各客户）；替换件合并 + 断点归属：`demand.build_gross` → `md_part_replace.list` + `md_breakpoint.list`
...
- **依赖输入**: 本表 `demand` 行（**本应用**：`build_gross` 产生）；版本状态（**上游**：`md_monthly_version` 产生）
- **输出**: `md_monthly_version.lock_status`（草稿 → 发布（锁定）→ 冻结）；发布后 `demand` 行不可改（由版本状态在 `build_gross` 处回拦）
- **生效点**: `demand.publish` → `md_monthly_version.publish`；`demand.calc_net` → `md_monthly_version.freeze`
...
- **依赖输入**: `version_no` + `material_no` + `rolling_month`（入参 + 汇总行）
- **输出**: `demand` 主键（`version_no` + `material_no` + `rolling_month`）唯一 —— 重算只更新既有行，不新增
- **生效点**: `demand.build_gross` → `_upsert_gross`（先查后写：命中更新，未命中插入）
...
- **依赖输入**: `material_no`（**主数据**：`md_material` 产生）；`version_no`（**主数据**：`md_monthly_version` 产生）
- **输出**: `demand.material_no`、`demand.version_no`
- **生效点**: `demand.build_gross` → `md_monthly_version.get`（版本存在性有校验）；⚠ **`material_no` 在服务侧没有校验点** —— 只靠前端下拉 + 对账器 C1 引用完整性兜底

#### 3.2.13 `BR-13` 合成范围非空（fail-closed）
- **规则描述**: `build_gross` 的合成范围为**空**时报错，不得返回「成功，共 0 个物料」。空范围 = 销售预测汇总为空（或 `get_summary` 返回了无法识别的结构）。
- **适用场景**: `build_gross`。
- **为什么**: 旧行为是静默成功 —— `_get_summary_rows` 只在**调用抛异常**时才报错，调用成功但返回空 / `None` / 未知结构时一律静默返回 `[]`，于是写完循环一次都没进，接口却回一句「毛需求合成成功，共 0 个物料」。**什么都没做却声称成功**，调用方无从察觉，与该组已修的三处静默降级（毛需求丢水位层、推移表全标「无」、预测汇总被当 0）是同一类问题。
- **口径对照**: `master_plan` 的 BR-10「过滤后无命中行时报错，不静默导入 0 行」—— 同类操作两个应用口径必须一致（此前只有 master_plan 报了错）。
- **实测**: 2026-09-15 清掉该版本汇总行后调 `build_gross`，返回 `{'material_count': 0, 'message': '毛需求合成成功，共 0 个物料'}`。
- **发现方式**: `scripts/verify_psc_oracles.py` 的 F1 静默降级反证。
- **依赖输入**: 汇总行本身（**上游**：`sales_forecast.get_summary` 产生）—— **为空即报错**
- **输出**: 无（拒绝：抛 `FdeError`，不返回「成功，共 0 个物料」）
- **生效点**: `demand.build_gross`（步骤 3.5 的非空校验）
