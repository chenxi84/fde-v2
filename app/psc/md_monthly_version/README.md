...
月度版本是产销协同体系的「计划周期节奏载体」，为销售预测、库存策略、毛需求发布、净需求运算提供统一的版本键与「同一时刻仅一个活跃版本」的节奏边界。锁定状态 `lock_status` 流转：`草稿 --publish--> 发布（锁定） --freeze--> 冻结 --unfreeze--> 草稿`，冻结可经「回退」解冻回草稿（前提：无其他活跃版本）。
...
| `psc__md_monthly_version__freeze` | `version_no`：str，必填，无默认 | 将 `发布（锁定）` 版本冻结为 `冻结`（归档态）。仅发布（锁定）状态可冻结。返回更新后的版本对象。 |
| `psc__md_monthly_version__unfreeze` | `version_no`：str，必填，无默认 | 将 `冻结` 版本回退为 `草稿`（解冻）。仅冻结状态可回退；回退后重新成为活跃版本，须无其他活跃版本（BR-06）。返回更新后的版本对象。 |
...
4. 状态为 `冻结` 且需重新编辑时，调 `psc__md_monthly_version__unfreeze(version_no)` 回退到草稿（前提：无其他活跃版本）。
...
psc__md_monthly_version__unfreeze(version_no="202608")
```
...
3. **同一时刻仅一个活跃版本**
   ...
   - 若要开新版本，须先将当前活跃版本 `publish`（若为草稿）再 `freeze`（若已发布）至冻结归档态。
   - 回退冻结版本时同样受此约束：若已存在其他活跃版本，回退被拒（提示「已存在活跃版本」）。

4. **状态机流转**
   - `草稿 --publish--> 发布（锁定） --freeze--> 冻结 --unfreeze--> 草稿`，除回退外禁止逆向、禁止跳级。
   - `publish` 仅对 `草稿` 生效；`freeze` 仅对 `发布（锁定）` 生效；`unfreeze` 仅对 `冻结` 生效。
   - 冻结为归档态，可经 `unfreeze` 回退到草稿（前提：无其他活跃版本）；本应用不提供物理删除服务。
...
|---|---|---|
| `版本号不能为空` | `create` / `get` / `publish` / `freeze` / `unfreeze` 传入的 `version_no` 为空或全是空白。 | 向用户索要有效版本号（YYYYMM），或先调 `list` 查询取得版本号后重试。 |
...
| `已存在活跃版本` | `create` 时已存在草稿或发布（锁定）状态的活跃版本；或 `unfreeze` 回退冻结版本时已存在其他活跃版本。 | 先调 `get_active()` 找到当前活跃版本，将其 `publish`（若草稿）后 `freeze` 至冻结归档，再重新 `create` / `unfreeze`。 |
| `版本记录不存在` | `get` / `publish` / `freeze` / `unfreeze` 指定的 `version_no` 查不到记录。 | 先调 `list` 核对版本号是否拼写错误或尚未创建；确认后重试。 |
...
| `版本已冻结` | 对已 `冻结` 的版本再次调用了 `freeze`。 | 冻结版本无需重复冻结；如需重新编辑，先确认无其他活跃版本后调 `unfreeze` 回退到草稿。 |
| `仅冻结状态可回退` | 对 `草稿` 或 `发布（锁定）` 状态的版本调用了 `unfreeze`。 | 先调 `get` 确认当前状态；仅 `冻结` 可 `unfreeze`，草稿/发布（锁定）无需回退。 |
