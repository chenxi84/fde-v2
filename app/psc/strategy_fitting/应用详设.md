...
- **独立测试**：可通过在拟合页面选择某个物料、填入拟合版本后发起拟合，验证预测拟合（选 MASE 最小）与库存拟合均落表来独立测试
- **前置条件**: 物料在 md_material 中存在；该物料有足够历史干净需求（常规 ≥12 期 / 间歇 ≥4 期，否则预测方法留空、标记异常）
- **后置条件**: 生成一条状态为「待复核」的拟合结果记录，pred_method/pred_params/mase/smape/pred_qty/pred_lo/pred_hi/service_factor/safety_level/batch_window/fulfill_rate/inv_days/changeover_cnt 已填

**验收场景**：

1. `US-01-AC1` **场景**: 有足够历史的物料拟合成功
   - **Given** 物料 M 有 ≥12 期干净需求且 md_material 有参数，**When** 计划调用 run(M, 202608) 发起拟合，**Then** 系统完成预测拟合+库存拟合，落表 status=待复核，mase/pred_qty/服务系数等字段均已填写

2. `US-01-AC2` **场景**: 数据不足新品预测方法留空
   - **Given** 物料 N 历史不足（常规 <12 期 / 间歇 <4 期，新品/断点新件），**When** 发起 run(N, 202608)，**Then** 预测拟合跳过回测，pred_method 留空、abnormal_flag=true（数据不足，以人工拍数为准）
...
2. `US-03-AC2` **场景**: 按异常标记筛选
   - **Given** 部分拟合结果 abnormal_flag=true（候选为空或 MASE≥1），**When** 筛选异常标记，**Then** 列表仅展示需人工确认的异常记录
...
2. `US-05-AC2` **场景**: 异常记录强制人工确认
   - **Given** 待复核记录 abnormal_flag=true（候选为空或 MASE≥1），**When** 执行 approve，**Then** 系统要求人工二次确认后才允许生效，未确认则拒绝
...
6. 系统执行预测拟合：statsforecast 统计模型池 → cross_validation 滚动回测 → 选 MASE 最小
...
3. 若 abnormal_flag=true（候选为空或 MASE≥1），系统强制人工二次确认
...
- **依赖输入**: `material_no`（**入参**）；`fit_version`（**入参**）；同键既有行（**本表**：`strategy_fitting.run` 产生，`SELECT 1` 探测）
- **输出**: `strategy_fitting.material_no` + `strategy_fitting.fit_version`（schema 复合主键；重复拟合走 UPDATE 覆盖并重置 `status='待复核'`）
- **生效点**: `strategy_fitting.run`（`strategy_fitting.run_batch` 逐物料复用同一入口）
...
- **依赖输入**: 物料主数据 `md_material.batch_window` / `md_material.service_level`（**主数据**：计划员人工维护，`md_material.set_fit_params` 仅在复核通过后回填预测侧参数）
- **输出**: 中间量（`strategy_fitting.batch_window`、`strategy_fitting.fulfill_rate` 两个派生量，由 `_inventory_params(material)` 产出、**不接预测值**；`service_factor`/`safety_level`/`inv_days`/`changeover_cnt` 四项**恒为 NULL** —— 系统不再计算它们，如实留空而不填默认值）
- **生效点**: `strategy_fitting.run` → `_inventory_params`（与 `_predict_fit` 无数据依赖）

> ⚠ **2026-09-18 订正**：本条三要素原写「六个派生量由 `_inventory_fit(history)` 产出」—— 方法已按业务口径
> （见下方 BR-08~BR-12 整段说明与 `BR-20`）改名为 **`_inventory_params(material)`**，且**只产 2 个量**。
> 库存侧现在**不读历史序列**，与预测拟合的解耦比原来更彻底（连输入都不共享）。

#### 3.2.3 `BR-03` 滚动回测 Cross-Validation 只用当时已知数据
- **规则描述**: 用 `StatsForecast.cross_validation(h=1, n_windows=3)` 滚动回测——只用「当时已知」数据预测「当时未知」未来，避免用未来数据预测过去的作弊
- **适用场景**: 预测拟合
- **示例**: 每个 cutoff 点只用 cutoff 之前的序列拟合，预测其后 h=1 期
- **依赖输入**: 历史序列 `history` 与真实期间 `periods`（**外部适配器**：`_load_sales_history` 产生）；历史期数 `n`（**本表**：由 `history` 长度派生）
- **输出**: 中间量（`cross_validation` 结果 `cv`，回测窗口数 `n_windows = max(1, min(3, n − k_min))`，不落库）
- **生效点**: `strategy_fitting.run` → `_predict_fit`

#### 3.2.4 `BR-04` 误差评价 MASE 为主、sMAPE 参考
- **规则描述**: 每个模型回测误差 `MAE = mean(|pred − actual|)`；`MASE = MAE / SeasonalNaive_MAE`（分母 = 训练集内 seasonal naive 误差 `|y[t] − y[t−12]|`，经库函数 `utilsforecast.losses.mase` 计算，逐 cutoff 取均值）。MASE 越小越优；`MASE ≥ 1` 表示不比季节朴素强 → 不可预测。sMAPE 保留作参考
- **适用场景**: 预测拟合误差评价
- **示例**: 模型 A MAE=42、季节朴素 MAE=70 → MASE=0.6（优于基线）；模型 B MASE=1.3 → 不可预测
- **依赖输入**: 回测结果 `cv` 与训练集 `df`（**本表**：`_predict_fit` 内由 `cross_validation` 产生）；MASE 季节性标尺 `mase_seasonality`（**本表**：按 `n ≥ 12` 派生）
- **输出**: `strategy_fitting.mase`、`strategy_fitting.smape`（逐候选同时落 `strategy_fitting.detail_json` 的 `candidates[].mase`/`candidates[].smape`）
- **生效点**: `strategy_fitting.run` → `_predict_fit`（MASE 由库函数 `utilsforecast.losses.mase` 计算）

#### 3.2.5 `BR-05` 预测候选模型池（statsforecast 分流）
- **规则描述**: 预测拟合按历史非零占比分流选模型池与最小窗口：
  - 非零占比 ≥ 30%（常规）：`AutoTheta(12) / AutoARIMA(12) / AutoETS(12)`，需 ≥12 期
  - 非零占比 < 30%（间歇）：`CrostonOptimized / TSB`，需 ≥4 期
  - MASE 基线由库函数从训练集计算（常规/长序列 = 季节性朴素 12；短间歇 <12 期退化为 naive 1）
- **适用场景**: 预测拟合
- **示例**: 常规需求走三个自动模型竞标；间歇需求走 Croston/TSB 竞标，短历史（≥4 期）也能拟合
- **依赖输入**: 历史非零占比 `nonzero_ratio`（**本表**：`_predict_fit` 由 `history` 派生）；历史期数 `n`（**外部适配器**：`_load_sales_history` 产生）
- **输出**: 中间量（候选模型列表 `models` 与最小窗口 `k_min`/`mase_seasonality`，不落库；`k_min` 另存 `strategy_fitting.detail_json.k_min`）
- **生效点**: `strategy_fitting.run` → `_predict_fit`

#### 3.2.6 `BR-06` 预测拟合取 MASE 最小者为最优
- **规则描述**: 各候选模型回测后取 MASE 最小者为最优预测方法（pred_method/pred_params），逐候选记录 mase/smape
- **适用场景**: 预测拟合结果选择
- **示例**: 候选 A MASE=0.62、候选 B MASE=0.9，选 A
- **依赖输入**: 各候选的 `mase`（**本表**：`_predict_fit` 回测产生）
- **输出**: `strategy_fitting.pred_method`、`strategy_fitting.pred_params`（`candidates.sort(key=mase)` 后的 winner）；候选全量明细落 `strategy_fitting.detail_json.candidates`
- **生效点**: `strategy_fitting.run` → `_predict_fit` →（approve 回填后）`sales_forecast.calc_baseline`

#### 3.2.7 `BR-07` 数据不足跳过回测留空
- **规则描述**: 数据不足（常规 <12 期、间歇 <4 期），跳过回测，pred_method 留空、abnormal_flag=true（数据不足，以人工拍数为准）
- **适用场景**: 预测拟合
- **示例**: 新品历史仅 3 期（无论常规/间歇），无法回测，pred_method=None
- **依赖输入**: 历史期数 `n`（**外部适配器**：`_load_sales_history` 序列长度）；门槛 `k_min`（**本表**：`_predict_fit` 按非零占比分流产生）
- **输出**: `strategy_fitting.pred_method`（留空 NULL）、`strategy_fitting.abnormal_flag`（=1）、`strategy_fitting.mase`/`strategy_fitting.smape`（=0.0）
- **生效点**: `strategy_fitting.run` → `_predict_fit` → `_default_result`（`n == 0` 或 `n < k_min` 分支）

> **⛔ 2026-09-17 业务口径变更（BR-08~BR-12 整段）**：业务方确认 ——
> **组批窗口与满足率目标由物料主数据人工维护**（计划员判断「这个件该不该组批」，维护在主数据上），
> **不需要系统自动运算**。BRD §4.8.1 的字段表本身就把组批窗口写成
> 「**物料主数据（经验值）**或经济批量模型计算」，本次取前者。
>
> 因此下面 BR-08~BR-12 那**一套（网格遍历 / 回放 / 约束寻优 / 成本折算）本期不实现**，
> 各条的 `输出` / `生效点` 已按「本期不产出」改写（**不是「没做」，是「不做」** —— 留在这里是
> 为了保留完整设计意图，将来要做时按原文实施）。**真实口径见新增的 `BR-20`。**
>
> 之所以要写这么重：这五条此前**看起来是实现了的**（旧方法 `_inventory_fit`（今 `_inventory_params`）会返回 1.65 / 28.0 / 0.95
> 等一串数），实际是**编造的常量**，而且 `approve` 还把它们**回填进主数据** ——
> 等于「人工维护的组批窗口，跑一次拟合就被静默冲掉」。改口径的同时把那处覆盖也停掉了。
...
- **依赖输入**: 服务系数网格 {1.28, 1.65, 2.05, 2.33}、组批窗口网格 {2, 4, 8, 12, 16 周}（⚠ **系统内无产生方**：`_inventory_params` **不遍历**网格，只读主数据人工值，网格只存在于本文件与 BRD §7.5）
- **输出**: 无（**本期不产出**：网格遍历随库存侧不自动运算一并搁置；那两个字段现由 BR-20 记**主数据人工值**）
- **生效点**: 无（本期不做；将来实施时落在 `strategy_fitting.run` 的库存拟合节点）
...
- **依赖输入**: 实际需求序列 D[t]（**外部适配器**：`_load_sales_history` —— ⚠ 见 BR-19，实为台账原始 `SUM(qty)`，非剔除脉冲后的干净需求）；期初库存、最低水位 A（⚠ **系统内无产生方**：`run` 里由 `prod_days`+`logistics_days` 算出的 `lead_days` 只喂 σ_L，未参与库存计算，全仓无期初库存来源）
- **输出**: 无（**本期不产出**：回放不做 ⇒ `inv_days` / `changeover_cnt` **留空**；
  ⚠ 此前它们由简化公式 `batch_window + safety/daily` 与 `len(history)//4` **编造**，已停止编造）
- **生效点**: 无（本期不做）
  > 附带说明：本条输入里的「**期初库存**」在系统内**没有产生方**（全仓无来源）——
  > 也就是说即使要做回放，也**得先给它一个起点**。这是「不做」之外的另一层原因。
...
- **依赖输入**: 累计缺货量、总需求（⚠ **系统内无产生方**：无回放即无缺货量统计，`_inventory_params` 不计算该量）
- **输出**: 无（**本期不产出**：满足率不计算 ⇒ `fulfill_rate` 现记**主数据里的满足率目标**，见 BR-20）
- **生效点**: 无（本期不做；⚠ 此前 `approve` 会把常量 0.95 **回填进 `md_material.service_level`**，
  覆盖人工维护的目标值 —— 该回填已按新口径停止）
...
- **依赖输入**: 各候选的满足率与成本（⚠ **系统内无产生方**：候选遍历与成本比较在系统内不存在）；满足率目标（**入参**/规则常量，如 95%）
- **输出**: 无（**本期不产出**：不寻优 ⇒ `service_factor` / `safety_level` **留空**；
  `batch_window` 由 BR-20 记主数据人工值）
- **生效点**: 无（本期不做）
...
- **依赖输入**: 单位货值 `unit_value`、切线成本 `change_cost`（**主数据**：`md_material.get` 取，`md_material.create`/`update`/`import_batch` 产生）；年持有费率（⚠ **系统内无产生方**：无「策略配置」应用或配置项，全仓无该参数的任何读写点，仅存在于 BRD §8.8）
- **输出**: 中间量（持有成本 / 切线成本）—— **本期不产出**：不寻优就不需要成本评价，
  故 `unit_value` / `change_cost` 取到也不再使用（此前取到也从未被引用）
- **生效点**: 无（本期不做；成本折算的生效点缺失）

> ⚠ 生效点原本写 `strategy_fitting.run` → 旧方法 `_inventory_fit`（成本折算）。该方法已按新口径改名为
> `_inventory_params` 且**不再碰成本**（只读主数据两个参数），故这里不再指向任何调用链。
> 详设里提到旧方法名时一律**不加反引号**——三要素行上的反引号是「这是什么服务」的机器判据，
> 写成 `_inventory_fit` 会被判成「全组找不到这个方法」（2026-09-18 就是这样被 `br_meta_check` 逮到的）。
...
- **依赖输入**: 拟合产出（**本表**：`run` 的 `_predict_fit`/`_inventory_params` 产生）；复核人确认（**入参**：`approve` 的 `confirm`）
- **输出**: `strategy_fitting.status`（`待复核` → `已生效`）；回填跨应用写 `md_material.base_method`、`md_material.base_params`、`md_material.batch_window`、`md_material.service_level`、`md_material.fit_version`、`md_material.fit_effective_at`、`md_material.sigma_l`、`md_material.model_blob`
- **生效点**: `strategy_fitting.run` → `strategy_fitting.approve` → `md_material.set_fit_params`

#### 3.2.14 `BR-14` 拟合异常自动标记并强制人工确认
- **规则描述**: 拟合结果异常时自动标记（abnormal_flag=true），强制人工二次确认后方可生效。异常 = 候选为空（数据不足/拟合失败，pred_method 留空）或 winner MASE ≥ 1（不比季节朴素强）
- **适用场景**: approve（异常记录）
- **示例**: 数据不足 3 期 → 候选为空 → 标记异常；winner MASE=1.3 → 标记异常，approve 须二次确认
- **依赖输入**: 候选列表（**本表**：`_predict_fit` 产生，落 `strategy_fitting.detail_json.candidates`）；winner `mase`（**本表**：`strategy_fitting.mase`）
- **输出**: `strategy_fitting.abnormal_flag`（候选为空或 `mase ≥ 1` → 1，否则 0）
- **生效点**: `strategy_fitting.run`（标记）→ `strategy_fitting.approve`（`confirm` 未传时抛 `FdeError` 拦截）
...
- **依赖输入**: 本版回填参数（**本表**：本记录的 `strategy_fitting.pred_method`/`pred_params`/`batch_window`/`fulfill_rate`/`sigma_l`）；上一版已生效参数（**本表**：`strategy_fitting.approve` 产生的更早 `fit_version` 且 `status=已生效` 的行 —— 定位不走 `md_material_param_version`）
- **输出**: `md_material.fit_version`、`md_material.fit_effective_at` + `md_material_param_version` 快照行（`material_no`+`fit_version` 唯一，⚠ 该快照表当前无读取方）；回滚后本记录 `strategy_fitting.status` = `已否决`
- **生效点**: `strategy_fitting.approve` / `strategy_fitting.rollback` → `md_material.set_fit_params`

#### 3.2.16 `BR-16` 状态机流转约束
- **规则描述**: 拟合结果状态流转须遵循状态机（见 §3.4），非法跳转拒绝
- **适用场景**: approve / reject / rollback
- **示例**: 已否决记录不可再 approve；已生效记录不可再 approve
- **依赖输入**: 当前状态 `status`（**本表**：`strategy_fitting.run`/`approve`/`reject`/`rollback` 产生）
- **输出**: `strategy_fitting.status`（仅 `待复核→已生效`、`待复核→已否决`、`已生效→已否决` 三条合法边，`已否决` 为终态）；非法流转 无（拒绝：抛 `FdeError("仅待复核状态可生效")` / `FdeError("仅待复核状态可否决")` / `FdeError("仅已生效状态可回滚")`）
- **生效点**: `strategy_fitting.approve` / `strategy_fitting.reject` / `strategy_fitting.rollback`
...
- **依赖输入**: `material_no`（**入参**）；物料存在性与主数据（**主数据**：`md_material.get` 取，由 `md_material.create`/`md_material.import_batch` 产生）
- **输出**: 无（拒绝：抛 `FdeError("物料不存在")`）；通过时仅为内存中间量（`material` 主数据字典，不落库）
- **生效点**: `strategy_fitting.run` → `md_material.get`（`strategy_fitting.run_batch` 改走 `md_material.list(status="正常")` 取候选物料）

#### 3.2.18 `BR-18` 拟合历史窗口要求
- **规则描述**: 拟合需足够历史（常规 ≥12 期、间歇 ≥4 期）；不足者跳过回测、pred_method 留空、abnormal_flag=true（以人工拍数为准）
- **适用场景**: run 数据准备
- **示例**: 常规历史 ≥12 期可拟合；间歇历史 ≥4 期可拟合；更少则 pred_method 留空
- **依赖输入**: 历史期数 `n`（**外部适配器**：`_load_sales_history` 序列长度，默认取近 24 期）；分流结果与门槛 `k_min`（**本表**：`_predict_fit` 按非零占比产生）
- **输出**: 中间量（门槛判定 `n < k_min`，不落库）；不足时落 `strategy_fitting.pred_method`（留空 NULL）+ `strategy_fitting.abnormal_flag`（=1）
- **生效点**: `strategy_fitting.run` → `_predict_fit`（`n < k_min` 分支）

#### 3.2.19 `BR-19` 干净需求剔除一次性脉冲
- **规则描述**: 拟合前剔除一次性脉冲事件，否则脉冲会污染参数（σ、MAPE 均虚高）
- **适用场景**: run 数据准备（经 `_load_sales_history` 取「干净需求」）
- **示例**: 某月一次性促销脉冲不计入拟合历史
- **依赖输入**: 历史台账（**上游**：`sales_history.history_series` 产生，经 `md_material.history_chain` 拼前序链 ∪ 断点链）；⚠ **「已剔除一次性脉冲」的干净需求系统内无产生方** —— `history_series` 只做 `SUM(qty)` 原样聚合，全仓无任何脉冲剔除逻辑
- **输出**: 中间量（`history` 数量序列与 `periods` 期间序列，不落库；间接决定 `strategy_fitting.pred_qty`/`mase`/`sigma_l`）
- **生效点**: `strategy_fitting.run` → `_load_sales_history` → `md_material.history_chain` + `sales_history.history_series`

#### 3.2.20 `BR-20` 库存侧参数取主数据人工值（本期口径）
- **规则描述**: 组批窗口与满足率目标**由物料主数据人工维护**（计划员判断该件该不该组批 → 在主数据上设
  `batch_window` / `service_level`）；本应用在拟合时**只读回这两个值并记进拟合记录**，供复核页展示
  「这一版拟合所依据的库存参数」，**不回填、不覆盖、不计算**。
- **为什么单立一条**: 它是 BR-08~BR-12 那套自动寻优的**替代口径**（2026-09-17 业务确认）。立成显式的 BR，
  下游判据才盯得住 —— 否则「拟合记录里的窗口 == 主数据里的窗口」这条关系没有任何一条规则声明它。
- **示例**: `md_material.batch_window=28`（人工维护）→ 拟合记录的 `batch_window=28`；复核通过后
  `md_material.batch_window` **仍是 28**（拟合不改它）。
- **依赖输入**: `batch_window` / `service_level`（**主数据**：`md_material` 产生，人工维护）
- **输出**: `strategy_fitting.batch_window`、`strategy_fitting.fulfill_rate`（= 主数据当前值的**镜像**，供追溯）
- **生效点**: `strategy_fitting.run` → `_inventory_params(material)` → `md_material.get`；
  复核 `approve` / 回滚 `rollback` **只回填预测侧**（`md_material.set_fit_params` 的
  `batch_window` / `service_level` 已改为「给了才更新」，不传即保持原值）
...
- **数据描述**: 预测拟合选出的最优方法及其参数（JSON）；数据不足时 pred_method 留空（None）
- **数据类型**: 枚举值 / 文本(JSON)
- **数据来源**: 预测拟合（statsforecast 回测选 MASE 最小）自动计算
- **示例值**: AutoTheta / {"season_length":12}

#### 3.3.4 数据4: 预测误差 mase / smape
- **数据描述**: 最优预测方法的选型误差 MASE（主，越小越优、>1 不可预测）与 sMAPE（参考）
- **数据类型**: 数字
- **数据来源**: 预测拟合自动计算
- **示例值**: mase 0.62 / smape 0.152（15.2%）
...
| 待复核 | 已生效 | approve | 人工复核通过；abnormal_flag=false 或异常已人工二次确认；pred_method 非空时 md_material.set_fit_params 回填成功（为空则跳过回填） |
| 待复核 | 已否决 | reject | 人工否决 |
...
| 策略拟合 | F01 | 策略拟合 | F01-01 | 取最新一版拟合 | FUNC-08 | 接口 | P2 | get_latest：按物料取最新一版拟合全字段（供物料360聚合） |
...
7. 系统执行预测拟合：statsforecast 统计模型池（常规 AutoTheta/AutoARIMA/AutoETS，间歇 Croston/TSB，按非零占比分流）→ cross_validation 滚动回测 → 逐序列按 MASE 选 winner → winner 全量 refit → predict(h=1) 出下月预测 + 80% 区间 → 按响应窗口 L 再跑 h=L 回测算预测误差 σ_L
...
**输出数据**:
- 拟合结果记录: fit_version + material_no 定位，含 pred_method/pred_params/mase/smape/pred_qty/pred_lo/pred_hi/service_factor/safety_level/batch_window/fulfill_rate/inv_days/changeover_cnt，status=待复核
- 错误消息: "物料不存在"（数据不足不报错，pred_method 留空、abnormal_flag=true）

**业务规则**:
- BR-01: fit_version + material_no 唯一
- BR-02: 预测拟合与库存拟合解耦
- BR-03/BR-04/BR-05/BR-06/BR-07: 预测拟合算法（cross_validation、MASE 选型、模型池分流、取最小、数据不足留空）
...
- 数据不足新品/断点新件（常规 <12 期 / 间歇 <4 期）跳过回测，pred_method 留空、abnormal_flag=true（以人工拍数为准）
...
| pred_method | 枚举值 | 否 | - | 最优预测方法：AutoTheta/AutoARIMA/AutoETS/SeasonalNaive/CrostonOptimized/TSB；数据不足时留空（None）；预测拟合完成后回填 |
| pred_params | 文本 | 否 | - | 最优预测参数 JSON（如 {"season_length":12}）；预测拟合完成后回填 |
| mase | 数字 | 否 | - | 选型误差 MASE（BR-04，主，越小越优、>1 不可预测） |
| smape | 数字 | 否 | - | 预测误差 sMAPE（BR-04 参考），预测拟合完成后回填 |
| pred_qty | 数字 | 否 | - | 下月点预测值（winner 全量 refit → predict(h=1)） |
| pred_lo | 数字 | 否 | - | 下月预测 80% 区间下界 |
| pred_hi | 数字 | 否 | - | 下月预测 80% 区间上界 |
| sigma_l | 数字 | 否 | - | 响应窗口预测误差标准差 σ_L（statsforecast 按 h=L 回测累计误差），approve 回填 md_material 供库存策略安全库存口径 |
...
| abnormal_flag | 布尔值 | 否 | false | 拟合异常标记（BR-14）：候选为空或 MASE≥1 时为 true，approve 须人工二次确认 |
...
**输出数据**:
- 回填结果 + 记录状态变已生效
- 错误消息: "仅待复核状态可生效" / "拟合异常（数据不足或 MASE≥1 不可预测），需人工二次确认" / "回填失败"

**业务规则**:
- BR-13: 先落表、复核通过才回填
- BR-14: 拟合异常（数据不足或 MASE≥1）强制人工确认
- BR-15: 回填带版本可回滚
- BR-16: 状态机流转约束

**特殊处理**:
- 跨应用调用：`md_material.set_fit_params(...)` 回填参数
- 回填内容：基线方法/参数（pred_method/pred_params）、组批窗口（batch_window）、服务系数/满足率目标（service_factor/service_level）、安全水位（safety_level）；pred_method 为空（数据不足）时跳过回填，仅登记复核结论（物料保持空基线，销售预测以人工为准）
...
#### 6.1.1 标准1: 预测拟合算法正确性
- **关联用户故事**: US-01
- **关联业务规则**: BR-03, BR-04, BR-05, BR-06, BR-07
- **验证方式**: 给定已知历史序列，独立复算各模型回测 MASE，比对系统选出的最优模型与预测值
- **测试数据**: 某物料 ≥12 期干净需求
- **预期结果**: 系统分流选模型池、cross_validation 只用当时已知数据、取 MASE 最小者为 pred_method/pred_params 并出 pred_qty/pred_lo/pred_hi
...
- **关联用户故事**: US-05
- **关联业务规则**: BR-14
- **验证方式**: 构造数据不足（常规 <12 期 / 间歇 <4 期）或 winner MASE≥1 的拟合结果，执行 approve
- **测试数据**: 历史仅 3 期的物料（候选为空）或 MASE≥1 的物料
- **预期结果**: abnormal_flag=true，approve 被拦截要求人工二次确认，确认后生效（数据不足时 pred_method 为空则跳过回填）
...
- 常规最小窗口 k_min=12、间歇 k_min=4、回测窗口 n_windows ≤ 3（自适应）、近期窗口 recent_window=6 属「待定默认值」，实现时做成配置项（BRD §8.8）
- 数据不足（常规 <12 期 / 间歇 <4 期）物料无法拟合，pred_method 留空、以人工拍数为准
