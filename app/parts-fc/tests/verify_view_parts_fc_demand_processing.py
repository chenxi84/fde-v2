"""e2e 前端验收 - parts-fc:demand_processing（需求加工）
断言：路由渲染防粘滞 → 造数后列表有数据 → 模态全字段 → 表单落库回显 → 全程 0 报错。
运行：python app/parts-fc/tests/verify_view_parts_fc_demand_processing.py
"""
...
SEED_JS = r"""async () => {
  ...
  await call("project_ledger", "create", {
    project_no: "DP-PRJ-001", part_no: "DP-PART-001", oem_code: "OEM-A",
    plant_code: "PLANT-1", veh_model: "DP-MODEL-A", part_kind: "专用",
    sop: "2025-01", owner_sales: "S001"
  });
  await call("vehicle_part_map", "create", {
    part_no: "DP-PART-001", veh_model: "DP-MODEL-A", usage: 1, share: 100,
    lc_stage: "成熟", lc_shape: "传统", sop: "2025-01", source: "BOM"
  });

  const dp1 = await call("demand_processing", "create_batch", {
    fcst_version: "V2026-08", oem_code: "OEM-A", plant_code: "PLANT-1"
  });
  const dp2 = await call("demand_processing", "create_batch", {
    fcst_version: "V2026-09", oem_code: "OEM-B", plant_code: "PLANT-2"
  });

  const dp1Data = unwrap(dp1);
  const dp1Batch = dp1Data && (dp1Data.proc_batch || dp1Data.batch_no || dp1Data.no);

  // DP1 生成基线 + 提交修正 + 核对
  if (dp1Batch) {
    await call("demand_processing", "generate_baseline", {proc_batch: dp1Batch});
    await call("demand_processing", "submit_adjustment", {
      proc_batch: dp1Batch, part_no: "DP-PART-001", veh_model: "DP-MODEL-A",
      period: "2026-09", adj_qty: 1100, adj_reason_cat: "客户侧情报", adj_evidence: "客户确认函"
    });
    await call("demand_processing", "review", {
      proc_batch: dp1Batch, part_no: "DP-PART-001", veh_model: "DP-MODEL-A",
      period: "2026-09", chk_result: "通过", chk_reason: "修正合理"
    });
  }

  return {seeded: true, dp1_batch: dp1Batch};
}"""
...
def main():
    ...
    try:
        ...
        with sync_playwright() as p:
            ...
            # ===================== §1 本页渲染（防粘滞）=====================
            page.evaluate("location.hash = '#/demand_processing'")
            try:
            ...
            assert page.locator("main").inner_text().strip(), "demand_processing 页面无内容"
            assert page.locator("main .card").count() > 0, "demand_processing 未渲染 card"
            assert page.locator(".kpi:visible").count() == 0, "demand_processing 挂载未重建（残留看板）"

            # ===================== §0 造数 =====================
            seed = page.evaluate(SEED_JS)
            assert seed and seed.get("seeded"), f"demand_processing 造数失败：{seed}"
            ...
            assert row_count >= 1, f"demand_processing 列表行数不足：{row_count}"

            # ===================== §3 模态全字段 =====================
            if seed.get("dp1_batch"):
                dp1_batch = seed["dp1_batch"]
                page.locator(f'.b-link:has-text("{dp1_batch}"), a:has-text("{dp1_batch}")').first.click()
                modal = open_modal(page)
                txt = modal.inner_text()
                for expected in [dp1_batch, "OEM-A", "PLANT-1", "创建时间"]:
                    if expected:
                        assert expected in txt, f"demand_processing 详情模态缺字段：{expected}"
                close_modal(page, modal)

            # ===================== §4 表单落库回显 =====================
            click_button(page, ["+ 新建批次", "新建批次", "+ 新建", "新建"])
            modal = open_modal(page)
            ...
            # ===================== §6 0 报错 =====================
            assert not errors, f"demand_processing 会话前端报错：{errors[:5]}"
            print("VERIFY_VIEW_parts_fc_demand_processing: PASS")
