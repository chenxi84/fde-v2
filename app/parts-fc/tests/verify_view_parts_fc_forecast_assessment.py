"""e2e 前端验收 - parts-fc:forecast_assessment（预测考核）
断言：路由渲染防粘滞 → 造数后列表有数据 → 模态全字段 → 表单落库回显 → 全程 0 报错。
运行：python app/parts-fc/tests/verify_view_parts_fc_forecast_assessment.py
"""
...
SEED_JS = r"""async () => {
  ...
  await call("vehicle_part_map", "create", {
    part_no: "FA-PART-001", veh_model: "VEH-A", usage: 1, share: 100,
    lc_stage: "成熟", lc_shape: "传统", sop: "2024-03-01", source: "BOM"
  });
  await call("vehicle_part_map", "create", {
    part_no: "FA-PART-002", veh_model: "VEH-A", usage: 1, share: 100,
    lc_stage: "成熟", lc_shape: "传统", sop: "2024-03-01", source: "BOM"
  });

  const fa1 = await call("forecast_assessment", "create", {period: "2026-08"});
  const fa2 = await call("forecast_assessment", "create", {period: "2026-09"});

  const fa1Data = unwrap(fa1);
  const fa2Data = unwrap(fa2);
  const fa1No = fa1Data && (fa1Data.fa_no || fa1Data.no);
  const fa2No = fa2Data && (fa2Data.fa_no || fa2Data.no);

  if (fa1No) {
    await call("forecast_assessment", "add_record", {
      fa_no: fa1No, part_no: "FA-PART-001", veh_model: "FA-MODEL-A",
      period: "2026-08", fcst_qty: 1200, actual_qty: 1190,
      settle_qty: 1195, base_qty: 1180, adj_qty: 1200
    });
  }

  if (fa2No) {
    await call("forecast_assessment", "add_record", {
      fa_no: fa2No, part_no: "FA-PART-002", veh_model: "FA-MODEL-B",
      period: "2026-09", fcst_qty: 1000, actual_qty: 800,
      settle_qty: 810, base_qty: 950, adj_qty: 1050
    });
    try {
      await call("forecast_assessment", "attribute", {
        fa_no: fa2No, part_no: "FA-PART-002", period: "2026-09",
        attribution: "OEM需求塌方:客户实际订单量萎缩30%",
        result: "免责剔除", evidence: "客户确认函"
      });
    } catch (e) {}
  }

  return {seeded: true, fa1_no: fa1No, fa2_no: fa2No};
}"""
...
def main():
    ...
    try:
        ...
        with sync_playwright() as p:
            ...
            # ===================== §1 本页渲染（防粘滞）=====================
            page.evaluate("location.hash = '#/forecast_assessment'")
            try:
            ...
            assert page.locator("main").inner_text().strip(), "forecast_assessment 页面无内容"
            assert page.locator("main .card").count() > 0, "forecast_assessment 未渲染 card"
            assert page.locator(".kpi:visible").count() == 0, "forecast_assessment 挂载未重建（残留看板）"

            # ===================== §0 造数 =====================
            seed = page.evaluate(SEED_JS)
            assert seed and seed.get("seeded"), f"forecast_assessment 造数失败：{seed}"
            ...
            assert row_count >= 1, f"forecast_assessment 列表行数不足：{row_count}"

            # ===================== §3 模态全字段 =====================
            if seed.get("fa1_no"):
                fa1_no = seed["fa1_no"]
                page.locator(f'.b-link:has-text("{fa1_no}"), a:has-text("{fa1_no}")').first.click()
                modal = open_modal(page)
                txt = modal.inner_text()
                for expected in [fa1_no, "FA-PART-001", "2026-08", "创建时间"]:
                    if expected:
                        assert expected in txt, f"forecast_assessment 详情模态缺字段：{expected}"
                close_modal(page, modal)

            # ===================== §4 表单落库回显 =====================
            click_button(page, ["+ 新建考核", "新建考核", "+ 新建", "新建"])
            modal = open_modal(page)

            try:
                fill_labeled(modal, "期间", "2026-10")
            except Exception:
                fill_labeled(modal, "考核期", "2026-10")
            ...
            # ===================== §6 0 报错 =====================
            assert not errors, f"forecast_assessment 会话前端报错：{errors[:5]}"
            print("VERIFY_VIEW_parts_fc_forecast_assessment: PASS")
