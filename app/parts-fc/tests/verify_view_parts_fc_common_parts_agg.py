"""e2e 前端验收 - parts-fc:common_parts_agg（通用件汇总）
断言：路由渲染防粘滞 → 造数后列表有数据 → 模态全字段 → 表单落库回显 → 全程 0 报错。
运行：python app/parts-fc/tests/verify_view_parts_fc_common_parts_agg.py
"""
...
SEED_JS = r"""async () => {
  ...
  await call("vehicle_part_map", "create", {
    part_no: "CA-PART-001", veh_model: "VEH-A", usage: 1, share: 100,
    lc_stage: "成熟", lc_shape: "传统", sop: "2024-03-01", source: "BOM"
  });
  await call("vehicle_part_map", "create", {
    part_no: "CA-PART-002", veh_model: "VEH-A", usage: 1, share: 100,
    lc_stage: "成熟", lc_shape: "传统", sop: "2024-03-01", source: "BOM"
  });
  await call("vehicle_part_map", "create", {
    part_no: "CA-PART-003", veh_model: "VEH-A", usage: 1, share: 100,
    lc_stage: "成熟", lc_shape: "传统", sop: "2024-03-01", source: "BOM"
  });

  const ca1 = await call("common_parts_agg", "create", {
    part_no: "CA-PART-001", period: "2026-09"
  });
  const ca2 = await call("common_parts_agg", "create", {
    part_no: "CA-PART-002", period: "2026-09"
  });
  const ca3 = await call("common_parts_agg", "create", {
    part_no: "CA-PART-003", period: "2026-09"
  });

  const ca1Data = unwrap(ca1);
  const ca3Data = unwrap(ca3);
  const agg1No = ca1Data && (ca1Data.agg_no || ca1Data.no);
  const agg3No = ca3Data && (ca3Data.agg_no || ca3Data.no);

  if (agg1No) {
    await call("common_parts_agg", "add_detail", {
      agg_no: agg1No, oem_code: "OEM-A", owner_sales: "S001",
      approved_qty: 500, evidence_qty: 100, verbal_qty: 200
    });
    await call("common_parts_agg", "set_dedup", {
      agg_no: agg1No, dedup_qty: 700,
      dedup_reason: "口头加码200按历史折减至100，差异化折减依据充分且完整可供审计追溯"
    });
  }

  if (agg3No) {
    await call("common_parts_agg", "set_dedup", {
      agg_no: agg3No, dedup_qty: 1000,
      dedup_reason: "差异化折减：口头加码合计800经历史实际回测折减至750，理由充分依据完整"
    });
    await call("common_parts_agg", "confirm", {agg_no: agg3No});
  }

  return {seeded: true, agg1_no: agg1No, agg3_no: agg3No};
}"""
...
def main():
    ...
    try:
        ...
        with sync_playwright() as p:
            ...
            # ===================== §1 本页渲染（防粘滞）=====================
            page.evaluate("location.hash = '#/common_parts_agg'")
            try:
            ...
            assert page.locator("main").inner_text().strip(), "common_parts_agg 页面无内容"
            assert page.locator("main .card").count() > 0, "common_parts_agg 未渲染 card"
            assert page.locator(".kpi:visible").count() == 0, "common_parts_agg 挂载未重建（残留看板）"

            # ===================== §0 造数 =====================
            seed = page.evaluate(SEED_JS)
            assert seed and seed.get("seeded"), f"common_parts_agg 造数失败：{seed}"
            ...
            assert row_count >= 1, f"common_parts_agg 列表行数不足：{row_count}"

            # ===================== §3 模态全字段 =====================
            if seed.get("agg1_no"):
                agg1_no = seed["agg1_no"]
                page.locator(f'.b-link:has-text("{agg1_no}"), a:has-text("{agg1_no}")').first.click()
                modal = open_modal(page)
                txt = modal.inner_text()
                for expected in [agg1_no, "CA-PART-001", "2026-09", "创建时间"]:
                    if expected:
                        assert expected in txt, f"common_parts_agg 详情模态缺字段：{expected}"
                close_modal(page, modal)

            # ===================== §4 表单落库回显 =====================
            click_button(page, ["+ 新建汇总", "新建汇总", "+ 新建", "新建"])
            modal = open_modal(page)

            fill_labeled(modal, "零件号", "CA-NEW-001")
            fill_labeled(modal, "期间", "2026-10")
            ...
            # ===================== §6 0 报错 =====================
            assert not errors, f"common_parts_agg 会话前端报错：{errors[:5]}"
            print("VERIFY_VIEW_parts_fc_common_parts_agg: PASS")
