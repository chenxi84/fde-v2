"""e2e 前端验收 - parts-fc:independent_event（独立事件）
断言：路由渲染防粘滞 → 造数后列表有数据 → 模态全字段 → 表单落库回显 → 全程 0 报错。
运行：python app/parts-fc/tests/verify_view_parts_fc_independent_event.py
"""
...
SEED_JS = r"""async () => {
  ...
  await call("vehicle_part_map", "create", {
    part_no: "IE-PART-001", veh_model: "VEH-A", usage: 1, share: 100,
    lc_stage: "成熟", lc_shape: "传统", sop: "2024-03-01", source: "BOM"
  });
  await call("vehicle_part_map", "create", {
    part_no: "IE-PART-002", veh_model: "VEH-A", usage: 1, share: 100,
    lc_stage: "成熟", lc_shape: "传统", sop: "2024-03-01", source: "BOM"
  });
  await call("vehicle_part_map", "create", {
    part_no: "IE-PART-003", veh_model: "VEH-A", usage: 1, share: 100,
    lc_stage: "成熟", lc_shape: "传统", sop: "2024-03-01", source: "BOM"
  });

  const ie1 = await call("independent_event", "create", {
    event_type: "水位脉冲", part_no: "IE-PART-001", period: "2026-09",
    event_qty: 500, source_basis: "阶跃检测", oem_code: "OEM-A", veh_model: "MODEL-A"
  });
  const ie2 = await call("independent_event", "create", {
    event_type: "水位脉冲", part_no: "IE-PART-001", period: "2026-09",
    event_qty: 300, source_basis: "发运-结算倒推", oem_code: "OEM-A", veh_model: "MODEL-A"
  });
  const ie3 = await call("independent_event", "create", {
    event_type: "其他", part_no: "IE-PART-002", period: "2026-09",
    event_qty: -200, source_basis: "人工登记+说明", oem_code: "OEM-B", veh_model: "MODEL-B"
  });
  const ie4 = await call("independent_event", "create", {
    event_type: "断点·新件启动", part_no: "IE-PART-003", period: "2026-10",
    event_qty: 800, source_basis: "设变通知", oem_code: "OEM-A", veh_model: "MODEL-C",
    bp_batch: "BP-202610-01", bp_date: "2026-10-15"
  });

  const ie2Data = unwrap(ie2);
  const ie4Data = unwrap(ie4);
  const ie2No = ie2Data && (ie2Data.event_no || ie2Data.no);
  const ie4No = ie4Data && (ie4Data.event_no || ie4Data.no);

  if (ie2No) {
    await call("independent_event", "confirm", {event_no: ie2No});
  }
  if (ie4No) {
    await call("independent_event", "cancel", {event_no: ie4No, close_basis: "测试取消"});
  }

  return {seeded: true, ie1_data: unwrap(ie1), ie2_no: ie2No};
}"""
...
def main():
    ...
    try:
        ...
        with sync_playwright() as p:
            ...
            # ===================== §1 本页渲染（防粘滞）=====================
            page.evaluate("location.hash = '#/independent_event'")
            try:
            ...
            assert page.locator("main").inner_text().strip(), "independent_event 页面无内容"
            assert page.locator("main .card").count() > 0, "independent_event 未渲染 card"
            assert page.locator(".kpi:visible").count() == 0, "independent_event 挂载未重建（残留看板）"

            # ===================== §0 造数 =====================
            seed = page.evaluate(SEED_JS)
            assert seed and seed.get("seeded"), f"independent_event 造数失败：{seed}"
            ...
            assert row_count >= 1, f"independent_event 列表行数不足：{row_count}"

            # ===================== §3 模态全字段 =====================
            if seed.get("ie2_no"):
                ie2_no = seed["ie2_no"]
                page.locator(f'.b-link:has-text("{ie2_no}"), a:has-text("{ie2_no}")').first.click()
                modal = open_modal(page)
                txt = modal.inner_text()
                for expected in [ie2_no, "水位脉冲", "OEM-A", "创建时间"]:
                    if expected:
                        assert expected in txt, f"independent_event 详情模态缺字段：{expected}"
                close_modal(page, modal)

            # ===================== §4 表单落库回显 =====================
            click_button(page, ["单条事件登记", "+ 新建", "新建"])
            modal = open_modal(page)

            fill_labeled(modal, "零件号", "IE-NEW-001")
            ...
            # ===================== §6 0 报错 =====================
            assert not errors, f"independent_event 会话前端报错：{errors[:5]}"
            print("VERIFY_VIEW_parts_fc_independent_event: PASS")
