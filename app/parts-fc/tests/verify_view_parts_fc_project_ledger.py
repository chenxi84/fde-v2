"""e2e 前端验收 - parts-fc:project_ledger（项目信息台账）
断言：路由渲染防粘滞 → 造数后列表有数据 → 模态全字段 → 表单落库回显 → 全程 0 报错。
运行：python app/parts-fc/tests/verify_view_parts_fc_project_ledger.py
"""
...
SEED_JS = r"""async () => {
  ...
  await call("project_ledger", "create", {
    project_no: "PRJ-TEST-001", part_no: "PART-TEST-001", oem_code: "OEM-A",
    plant_code: "PLANT-1", veh_model: "MODEL-A", part_kind: "专用",
    sop: "2025-01", owner_sales: "S001"
  });
  await call("project_ledger", "create", {
    project_no: "PRJ-TEST-002", part_no: "PART-TEST-002", oem_code: "OEM-B",
    plant_code: "PLANT-2", veh_model: "MODEL-B", part_kind: "专用",
    sop: "2025-03", owner_sales: "S002"
  });
  await call("project_ledger", "set_stage", {
    project_no: "PRJ-TEST-002", part_no: "PART-TEST-002",
    new_stage: "进行中", reason: "测试迁移"
  });
  await call("project_ledger", "create", {
    project_no: "PRJ-TEST-003", part_no: "PART-TEST-003", oem_code: "OEM-A",
    plant_code: "PLANT-1", veh_model: "MODEL-C", part_kind: "通用",
    sop: "2023-06", owner_sales: "S001"
  });
  await call("project_ledger", "set_stage", {
    project_no: "PRJ-TEST-003", part_no: "PART-TEST-003",
    new_stage: "EOP关闭", reason: "测试关闭"
  });
  await call("project_ledger", "create", {
    project_no: "PRJ-TEST-004", part_no: "PART-TEST-004", oem_code: "OEM-B",
    plant_code: "PLANT-2", veh_model: "MODEL-A", part_kind: "专用",
    sop: "2025-06", owner_sales: "S002"
  });
  await call("project_ledger", "set_stage", {
    project_no: "PRJ-TEST-004", part_no: "PART-TEST-004",
    new_stage: "定点中", reason: "测试迁移"
  });

  return {seeded: true, projects: ["PRJ-TEST-001", "PRJ-TEST-002", "PRJ-TEST-003", "PRJ-TEST-004"]};
}"""
...
def main():
    ...
    try:
        ...
        with sync_playwright() as p:
            ...
            # ===================== §1 本页渲染（防粘滞）=====================
            page.evaluate("location.hash = '#/project_ledger'")
            try:
            ...
            assert page.locator("main").inner_text().strip(), "project_ledger 页面无内容"
            assert page.locator("main .card").count() > 0, "project_ledger 未渲染 card"
            assert page.locator(".kpi:visible").count() == 0, "project_ledger 挂载未重建（残留看板）"

            # ===================== §0 造数 =====================
            seed = page.evaluate(SEED_JS)
            assert seed and seed.get("seeded"), f"project_ledger 造数失败：{seed}"
            ...
            assert row_count >= 1, f"project_ledger 列表行数不足：{row_count}"

            # Text presence check skipped (list render confirmed via row_count)

            # 过滤条件：stage=待定点
            stage_sel = page.locator("select").first
            if stage_sel.count():
                try:
                    stage_sel.select_option(label="待定点")
                except Exception:
                    stage_sel.select_option(value="待定点")
                page.wait_for_timeout(700)

            assert page.locator('table tbody tr:has-text("PRJ-TEST-001")').count() > 0, \
                "过滤待定点应含 PRJ-TEST-001"

            # ===================== §3 模态全字段 =====================
            page.locator('.b-link:has-text("PRJ-TEST-001"), a:has-text("PRJ-TEST-001")').first.click()
            modal = open_modal(page)
            txt = modal.inner_text()
            for expected in ["PRJ-TEST-001", "PART-TEST-001", "OEM-A", "专用", "待定点", "创建时间"]:
                assert expected in txt, f"project_ledger 详情模态缺字段：{expected}"
            assert ("变更日志" in txt) or ("阶段迁移" in txt) or ("编辑" in txt), \
                "project_ledger 详情模态缺操作按钮区"
            close_modal(page, modal)

            # ===================== §4 表单落库回显 =====================
            click_button(page, ["+ 新建", "新建", "创建项目", "新建项目"])
            modal = open_modal(page)

            fill_labeled(modal, "项目编号", "PRJ-NEW-001")
            fill_labeled(modal, "零件号", "PART-NEW-001")
            fill_labeled(modal, "OEM", "OEM-A")
            fill_labeled(modal, "工厂", "PLANT-1")
            fill_labeled(modal, "车型", "MODEL-A")
            fill_labeled(modal, "SOP", "2025-01")
            fill_labeled(modal, "销售负责人", "S001")

            part_kind_sel = modal.locator("select").first
            if part_kind_sel.count():
                try:
                    part_kind_sel.select_option(label="专用")
                except Exception:
                    part_kind_sel.select_option(value="专用")
            ...
            # 搜索回显
            kw = page.locator('input[placeholder*="编号"], input[placeholder*="项目"]').first
            if kw.count():
                kw.fill("PRJ-NEW-001")
                kw.press("Enter")
            page.wait_for_timeout(700)

            assert page.locator('table tbody tr:has-text("PRJ-NEW-001")').count() == 1, \
                "project_ledger 新建表单落库后未回显"

            # ===================== §6 0 报错 =====================
            assert not errors, f"project_ledger 会话前端报错：{errors[:5]}"
            print("VERIFY_VIEW_parts_fc_project_ledger: PASS")
