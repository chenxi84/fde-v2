"""e2e 前端验收 - parts-fc:bullwhip_correction（牛鞭修正）
断言：路由渲染防粘滞 → 造数后列表有数据 → 模态全字段 → 表单落库回显 → 全程 0 报错。
运行：python app/parts-fc/tests/verify_view_parts_fc_bullwhip_correction.py
"""
...
SEED_JS = r"""async () => {
  ...
  await call("vehicle_part_map", "create", {
    part_no: "BW-PART-001", veh_model: "VEH-A", usage: 1, share: 100,
    lc_stage: "成熟", lc_shape: "传统", sop: "2024-03-01", source: "BOM"
  });
  await call("vehicle_part_map", "create", {
    part_no: "BW-PART-002", veh_model: "VEH-A", usage: 1, share: 100,
    lc_stage: "成熟", lc_shape: "传统", sop: "2024-03-01", source: "BOM"
  });
  await call("vehicle_part_map", "create", {
    part_no: "BW-PART-003", veh_model: "VEH-A", usage: 1, share: 100,
    lc_stage: "成熟", lc_shape: "传统", sop: "2024-03-01", source: "BOM"
  });

  const bw1 = await call("bullwhip_correction", "create", {
    part_no: "BW-PART-001", period: "2026-09", tier: "Tier1直供",
    derived_qty: 990, end_qty: 970, end_source: "结算"
  });
  const bw2 = await call("bullwhip_correction", "create", {
    part_no: "BW-PART-002", period: "2026-09", tier: "Tier2",
    derived_qty: 1500, end_qty: 1000, end_source: "OEM消耗"
  });
  const bw3 = await call("bullwhip_correction", "create", {
    part_no: "BW-PART-003", period: "2026-09", tier: "Tier1直供",
    derived_qty: 500, end_qty: 490, end_source: "结算"
  });

  const bw1Data = unwrap(bw1);
  const bw2Data = unwrap(bw2);
  const bw3Data = unwrap(bw3);
  const bw1No = bw1Data && (bw1Data.bw_no || bw1Data.no);
  const bw2No = bw2Data && (bw2Data.bw_no || bw2Data.no);
  const bw3No = bw3Data && (bw3Data.bw_no || bw3Data.no);

  if (bw1No) {
    await call("bullwhip_correction", "maintain", {bw_no: bw1No});
  }

  return {seeded: true, bw1_no: bw1No, bw2_no: bw2No, bw3_no: bw3No};
}"""
...
def main():
    ...
    try:
        ...
        with sync_playwright() as p:
            ...
            # ===================== §1 本页渲染（防粘滞）=====================
            page.evaluate("location.hash = '#/bullwhip_correction'")
            try:
            ...
            assert page.locator("main").inner_text().strip(), "bullwhip_correction 页面无内容"
            assert page.locator("main .card").count() > 0, "bullwhip_correction 未渲染 card"
            assert page.locator(".kpi:visible").count() == 0, "bullwhip_correction 挂载未重建（残留看板）"

            # ===================== §0 造数 =====================
            seed = page.evaluate(SEED_JS)
            assert seed and seed.get("seeded"), f"bullwhip_correction 造数失败：{seed}"
            ...
            assert row_count >= 1, f"bullwhip_correction 列表行数不足：{row_count}"

            # ===================== §3 模态全字段 =====================
            if seed.get("bw2_no"):
                bw2_no = seed["bw2_no"]
                page.locator(f'.b-link:has-text("{bw2_no}"), a:has-text("{bw2_no}")').first.click()
                modal = open_modal(page)
                txt = modal.inner_text()
                for expected in [bw2_no, "BW-PART-002", "Tier2", "创建时间"]:
                    if expected:
                        assert expected in txt, f"bullwhip_correction 详情模态缺字段：{expected}"
                close_modal(page, modal)

            # ===================== §4 表单落库回显 =====================
            click_button(page, ["+ 新建处理单", "新建处理单", "+ 新建", "新建"])
            modal = open_modal(page)

            fill_labeled(modal, "零件号", "BW-NEW-001")
            ...
            # ===================== §6 0 报错 =====================
            assert not errors, f"bullwhip_correction 会话前端报错：{errors[:5]}"
            print("VERIFY_VIEW_parts_fc_bullwhip_correction: PASS")
