...
class BaselineBorrowing:
    ...
    与 strategy_fitting 分工：历史充足→D10 选策略；历史不足→本表借基线。
    """

    def _init_db(self):
        self.db.execute("""CREATE TABLE IF NOT EXISTS baseline_borrowing (
        ...
        self.db.execute("""CREATE INDEX IF NOT EXISTS idx_bb_part_no ON baseline_borrowing (part_no);""")
        self.db.execute("""CREATE INDEX IF NOT EXISTS idx_bb_status ON baseline_borrowing (status);""")
        self.db.execute("""CREATE INDEX IF NOT EXISTS idx_bb_proc_batch ON baseline_borrowing (proc_batch);""")
    ...
    def create(self, part_no: str, veh_model: str, hist_months: int,
        ...
        - 可能调用 veh_part_map.get 获取形态/生命周期信息
        """
        ...
        # 可选跨应用调用 veh_part_map 获取形态/生命周期信息
        try:
            self.fde.call("veh_part_map", "get", part_no=part_no, veh_model=veh_model)
        except FdeError:
            pass  # 弱依赖，不阻断创建
        except Exception:
            pass
    ...
    def list(self, part_no: str = None, status: str = None,
             borrow_method: str = None, page: int = None, size: int = None):
        """按条件列表查询。"""
    ...
    def get_derived_baseline(self, jy_no: str):
        ...
        # 历史转充足检查（在返回中添加提示）
        note = ""
        if record.get("hist_months", 0) >= 12:
            note = "该零件可用历史已≥12个月，建议切换至D10自产策略并关闭本借用单"
        return {
            "jy_no": jy_no,
