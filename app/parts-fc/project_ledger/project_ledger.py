...
class ProjectLedger:
    ...
    usage/share 为引用展示字段，权威源在 veh_part_map。
    """

    def _init_db(self):
        self.db.execute("""CREATE TABLE IF NOT EXISTS prj_ledger (
                project_no      TEXT NOT NULL,
                part_no         TEXT NOT NULL,
                stage           TEXT NOT NULL DEFAULT '待定点',
                status          TEXT NOT NULL DEFAULT '待启用',
        ...
        # 兼容旧表
        try:
            self.db.execute("ALTER TABLE prj_ledger ADD COLUMN status TEXT NOT NULL DEFAULT '待启用'")
        except Exception:
            pass
        try:
            self.db.execute("ALTER TABLE prj_ledger ADD COLUMN replacement_for TEXT")
        except Exception:
            pass
        self.db.execute("""CREATE TABLE IF NOT EXISTS change_log (
                project_no  TEXT NOT NULL,
                part_no     TEXT NOT NULL,
    ...
    def create(self, project_no: str, part_no: str, stage: str = "待定点",
               status: str = "待启用", replacement_for: str = None,
        ...
        status = self._clean(status) or "待启用"
        replacement_for = self._clean(replacement_for) if replacement_for else None

        self.db.execute(
            "INSERT INTO prj_ledger (project_no, part_no, stage, status, replacement_for, award_prob, oem_code, "
            "plant_code, veh_model, platform, part_kind, usage, share, sop, eop, lc_shape, owner_sales) "
            "VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
            (project_no, part_no, stage, status, replacement_for, award_prob, oem_code, plant_code, veh_model,
    ...
    def get(self, project_no: str, part_no: str):
        ...
        if not project_no or not part_no:
            raise FdeError("项目号和零件号必填")
        row = self.db.execute(
            "SELECT * FROM prj_ledger WHERE project_no = ? AND part_no = ?",
            (project_no, part_no)
        ).fetchone()
        if not row:
    ...
    def list(self, stage: str = None, oem_code: str = None, owner_sales: str = None, page: int = None, size: int = None):
        """按条件列表查询。"""
        sql = "SELECT * FROM prj_ledger WHERE 1=1"
        params = []
        if stage:
            sql += " AND stage = ?"
            params.append(stage)
    ...
    def update(self, project_no: str, part_no: str, **kwargs):
        ...
        allowed_fields = [
            "stage", "status", "replacement_for", "award_prob", "oem_code", "plant_code", "veh_model",
        ...
        self.db.execute(
            f"UPDATE prj_ledger SET {', '.join(set_clauses)} WHERE project_no = ? AND part_no = ?",
            update_params
        )
    ...
    def get_replacement_for(self, part_no: str):
        """查询某零件是否是替换物料，返回被替换的物料号。"""
        part_no = self._clean(part_no)
        row = self.db.execute(
            "SELECT replacement_for FROM prj_ledger WHERE part_no=? AND replacement_for IS NOT NULL AND replacement_for != '' LIMIT 1",
            (part_no,)
        ).fetchone()
        if row:
            return {"part_no": part_no, "replacement_for": row["replacement_for"]}
        return {"part_no": part_no, "replacement_for": None}

    def get_active_projects(self, oem_code: str = None):
        """取进行中+正常零件清单——供 D03 自动填充（阶段=进行中，状态=正常）。"""
        sql = "SELECT * FROM prj_ledger WHERE stage = '进行中' AND status = '正常'"
        params = []
        if oem_code:
            sql += " AND oem_code = ?"
            params.append(oem_code)
    ...
    def _exists(self, project_no, part_no):
        return self.db.execute(
            "SELECT 1 FROM prj_ledger WHERE project_no = ? AND part_no = ?",
            (project_no, part_no)
