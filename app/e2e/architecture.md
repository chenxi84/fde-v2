...
> 上游：`app/e2e/brd/`（业务需求，第 1~5 章）｜逐条对账结论见 `app/e2e/architecture_review.md`
> ⚠ `brd/` 是 2026-09-18 据本文件与两份《应用详设》**反向整理**的（本组是样板组，没有真实业务方递交的 BRD），
> 来源与局限写在 `brd/00-目录.md` 与 `architecture_review.md` §五。
...
## 4. 跨应用调用方向（**声明表 · 唯一对账入口**）

> 正本格式见 `design-plus/架构设计.md` 的「输出结构 §②b」；由 `scripts/verify_app_edges.py`
> 与代码对账（**后端 `self.fde.call`** ↔ **前端页面字面量 `svc(...)`** 分开归类）。
> ⚠ 下面第 2 节各卡片里的 `#### 跨应用调用` 是**卡片视角的说明**，不是对账入口 ——
> 「在哪里调」这件事只有本表说得到（本组是样板组，样板必须带本表，否则照抄的组都不会有）。

**跨应用调用方向**（全部为**读**）：

| 调用方 | 被调用 | 用途 | 在哪一层 |
|---|---|---|---|
| `task` | `member.get` | 校验指派成员是否存在（弱引用存在性校验） | **后端** `self.fde.call` |
| `task` | `member.list` | 「负责人」候选（值仍是**文本**，后端不校验） | **视图层**（页面 `svc(...)`） |
| `dashboard`（组级聚合页） | `task.list` / `member.list` | 看板的 KPI 与状态分布 | **视图层**（组级页，按设计聚合全组） |
