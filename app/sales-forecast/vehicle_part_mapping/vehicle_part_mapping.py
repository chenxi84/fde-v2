...
class VehiclePartMapping:
    """车型—零件映射表——车型级外部数据到零件级的桥接，量纲（单车用量/份额）的权威源。

    标识（主键）：part_no + veh_model（联合业务键）
    含量纲历史版本子表——基线校准、FVA 复盘需要"当时的口径"。
    单一权威原则：用量/份额以本表为准，project_info 仅引用展示。
    """

    def _init_db(self):
        self.db.executescript("""
            CREATE TABLE IF NOT EXISTS vehicle_part_mapping (
    ...
    def create(self, part_no: str, veh_model: str, platform: str = None,
        ...
        self.db.execute(
            "INSERT INTO vehicle_part_mapping (part_no, veh_model, platform, usage, share, "
    ...
    def get(self, part_no: str, veh_model: str):
        ...
        if not part_no or not veh_model:
            raise FdeError("零件号和车型必填")
        row = self.db.execute(
            "SELECT * FROM vehicle_part_mapping WHERE part_no = ? AND veh_model = ?",
            (part_no, veh_model)
        ).fetchone()
        if not row:
    ...
    def list(self, part_no: str = None, veh_model: str = None,
             lc_stage: str = None, status: str = None):
        """按条件列表查询。"""
        sql = "SELECT * FROM vehicle_part_mapping WHERE 1=1"
        params = []
        if part_no:
            sql += " AND part_no = ?"
    ...
    def update(self, part_no: str, veh_model: str, **kwargs):
        ...
        self.db.execute(
            f"UPDATE vehicle_part_mapping SET {', '.join(set_clauses)} WHERE part_no = ? AND veh_model = ?",
            params
        )
        return self.get(part_no, veh_model)
    ...
    def set_usage(self, part_no: str, veh_model: str, new_usage: float,
        ...
        # 更新主表
        self.db.execute(
            "UPDATE vehicle_part_mapping SET usage = ? WHERE part_no = ? AND veh_model = ?",
    ...
    def set_share(self, part_no: str, veh_model: str, new_share: float,
        ...
        # 更新主表
        self.db.execute(
            "UPDATE vehicle_part_mapping SET share = ? WHERE part_no = ? AND veh_model = ?",
    ...
    def disable(self, part_no: str, veh_model: str, reason: str = ""):
        ...
        if current["status"] == "停用":
            return current
        self.db.execute(
            "UPDATE vehicle_part_mapping SET status = '停用' WHERE part_no = ? AND veh_model = ?",
            (part_no, veh_model)
        )
        return self.get(part_no, veh_model)
    ...
    def _exists(self, part_no, veh_model):
        return self.db.execute(
            "SELECT 1 FROM vehicle_part_mapping WHERE part_no = ? AND veh_model = ?",
            (part_no, veh_model)
