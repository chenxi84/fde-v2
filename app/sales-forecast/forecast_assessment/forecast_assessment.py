...
class ForecastAssessment:
    ...
    def create(self, period: str):
        ...
        self.db.execute(
            "INSERT INTO d12 (fa_no, part_no, veh_model, period, fcst_qty, actual_qty, settle_qty, base_qty, adj_qty, cycle) VALUES (?,?,?,?,?,?,?,?,?,?)",
            (fa_no, "", "", period, 0, 0, 0, 0, 0, period)
        )
    ...
    def add_record(self, fa_no: str, part_no: str, veh_model: str, period: str,
        ...
        self.db.execute(
            """INSERT INTO d12 (fa_no, part_no, veh_model, period, fcst_qty, actual_qty,
               conv_rate, dev_qty, settle_qty, base_qty, adj_qty, cycle)
               VALUES (?,?,?,?,?,?,?,?,?,?,?,?)""",
            (fa_no, part_no, veh_model, period, fcst_qty, actual_qty, conv_rate, dev_qty,
             settle_qty, base_qty, adj_qty, period)
        )
    ...
    def calculate(self, period: str):
        ...
        for r in records:
            ...
            self.db.execute(
                "UPDATE d12 SET conv_rate=?, dev_qty=? WHERE fa_no=? AND part_no=? AND period=?",
    ...
    def attribute(self, fa_no: str, part_no: str, period: str, attribution: str, result: str, evidence: str = ""):
        ...
        self.db.execute(
            "UPDATE d12 SET attribution=?, result=?, assessor=? WHERE fa_no=? AND part_no=? AND period=?",
    ...
    def list(self, part_no: str = None, period: str = None, result: str = None):
