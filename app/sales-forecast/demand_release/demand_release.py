...
class DemandRelease:
    ...
    def _init_db(self):
        self.db.executescript("""
            CREATE TABLE IF NOT EXISTS d09_header (
            ...
            CREATE TABLE IF NOT EXISTS d09_detail (
    ...
    def create_draft(self, base_period: str):
        ...
        now = datetime.now()
        rel_no = f"REL-{now.strftime('%Y%m')}-{now.strftime('%d%H%M')}"
        # 版本号
        ...
        return {"rel_no": rel_no, "rel_version": rel_version, "base_period": base_period,
                "status": "草稿", "message": "草稿已创建，请调用 add_line 逐行填充发布明细"}
    ...
    def list(self, rel_version: str = None, status: str = None):
