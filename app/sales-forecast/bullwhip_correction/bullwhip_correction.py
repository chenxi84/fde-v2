...
class BullwhipCorrection:
    ...
    def _init_db(self):
        self.db.executescript("""
            CREATE TABLE IF NOT EXISTS d08 (
    ...
    def create(self, part_no: str, period: str, tier: str, derived_qty: float = None,
        ...
        # Tier1直供+已拆解→简化确认
        if tier == "Tier1直供" and amp_factor <= theta_amp:
            final_qty = derived_qty
            status = "维持"
            corr_action = "维持——放大已由拆解先行完成"
        elif amp_verdict == "容忍内·维持":
            final_qty = derived_qty
            status = "维持"
            corr_action = None
        else:
    ...
    def list(self, part_no: str = None, period: str = None, tier: str = None, status: str = None):
    ...
    def maintain(self, bw_no: str):
        ...
        row = self._get_row(bw_no)
        if row["status"] != "维持":
            raise FdeError(f"当前状态 {row['status']}，不可维持")
        self.db.execute(
    ...
    def confirm(self, bw_no: str):
        ...
        row = self._get_row(bw_no)
        if row["status"] != "已修正":
            raise FdeError(f"当前状态 {row['status']}，不可确认")
        self.db.execute(
            "UPDATE d08 SET status='已确认', checker=?, chk_date=date('now') WHERE bw_no=?",
            (self.ctx["userno"], bw_no)
        )
