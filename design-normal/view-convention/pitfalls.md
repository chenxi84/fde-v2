...
> **条目编号是稳定 ID**（被 `design/` 九步法各规格 / 本目录 `architecture.md` 按号引用），瘦身合并后留有**空号**，不重排——引用方按号直达，新增坑续用空号或往后追加。纯"架构事实"（系统当前如何工作，无反直觉踩坑点）已归入 `architecture.md` / `patterns.md`，见文末速查，不再当坑记录。
