...
- ⚠️ **`list` 方法签名与返回格式**：供前端 `pageable()` 消费的 `list` 方法**必须**接受 `page: int = None` 和 `size: int = None` 两个末尾参数（参数名 `size` 与前端 `pageable()` 传参一致），在查询后**先保存 `total = len(rows)`**，再按 `rows[start:start+size]` 做服务端切片，最后返回 `{"items": [...], "total": total}`（`total` 为切片前全量行数）。
