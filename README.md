...
- **流水线（构建期）**：`design/` 轻量九步法（后端五步 + 前端四步）+ `/groupbuild` 平台任务，从一段业务说明出发，自动产出**架构设计 → 详设 → 编码 → 测试用例/执行 → 契约冻结 → 前端四步**，端到端交付一个可运行系统。
...
│   ├── groupbuild使用说明.md   #   平台轨（/groupbuild 一键任务卡片用法与顺序）
│   ├── 架构设计.md             #   第①步：聚合根识别
...
推荐入口：平台 **/groupbuild** 页——建应用组后按九步法卡片（③架构 → ⑫前端测试执行）逐步点「生成」，后台任务自动执行生成 / 运行 / 报告（详见 `design/groupbuild使用说明.md`）。
...
编排总纲两轨：**手工执行轨**见 `design/工具链使用说明.md`（Claude Code / 工程师逐步执行）；**平台轨**见 `design/groupbuild使用说明.md`（**/groupbuild** 页逐步卡片化，UI 卡 ②–⑫，后台队列任务 + 断点续跑，产物按组下载/删除/重执行）。两轨同规格、同产物、可互换。
...
**第⑤步 BUG 修复回路**：提案（`bugfix`，只提案不改码）→ 页面逐条批准 → 应用已批准并重测（`bugfix-apply`，改前自动备份 + 重测对账消项）。
...
**可重入 / 可续跑**：每步幂等地「吃上游产物 → 写本步产物」；平台任务断点续跑（中断后重跑自断点继续）。上线后持续演进 = 单步重执行 + 第⑤步 BUG 修复回路。
...
| 上线后持续演进 | /groupbuild 第⑤步 BUG 修复回路（提案 → 批准 → 应用重测）+ 单步重执行 |
