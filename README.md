...
- **定位**：FDE v2 面向**小型环境**（演示 / 部门级 / 单机容器化），默认 SQLite；配 `DATABASE_URL` 可切 PostgreSQL。
- **DB 层增强是"提前"做的**：该项**原计划排在 3.0 之后**；因已有反馈希望把本项目用于**数据可靠性要求较高的生产环境**，
  已在 **v2.4.0** 提前落地（DML 编译层、部署侧连接池、以及配套的真 PG 验证资产）。
  决策、边界与「运维注意项」见 [`design-plus/CONVENTION.md`](design-plus/CONVENTION.md) §14。
